...
<td><b>整站备份 / 恢复（`./vanblog.sh restore <归档名>` 可一步恢复）</b></td><td>一个高压缩归档（zstd → xz → gzip 自动选择）打包<b>全部集合 + 评论库 + 图床/附件/自定义页面</b>；支持不解压查看清单、鉴权下载、上传恢复（逐集合原子替换 + 重建索引）</td><td><a href="docs/advanced/backup.md">备份</a></td></tr>
