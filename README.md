...
<!-- 旧锚点兼容：本节曾名为「与上游的关系」，外部书签与历史文档里可能还留着 `#与上游的关系`。
     产品内的链接（前台页脚、后台「关于」页）现在都直接指向本节真实标题 `#出处与许可`，
     这一行只为旧书签保留，可以安全删除，但删除前请确认没有外部引用。
     🔴 改本节标题时请同步改那两处链接 —— 它们各有一条守卫钉住"锚点目标在 README 里真实存在"
     （`packages/website/__tests__/footerAttribution.spec.ts`、`packages/admin/tests/unit/aboutPage.test.js`），
     改名而不同步会立刻红，不会再像以前那样静默变成死锚点。 -->
<a id="与上游的关系"></a>
