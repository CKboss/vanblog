...
需要注意的是，导入导出并不会实际导出图床中的图片本身，而是导入导出其图片记录以便检索。本地图床时可参考下面的导出全部图片进行导出备份。[附件](../features/attachment.md)同理：JSON 导入导出只含记录，文件本身请用「附件管理 → 导出全部附件」或直接备份 `/app/static/file/` 目录。
