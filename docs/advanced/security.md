...
| 图片隐写检测 | 每 IP 每分钟 **10** 次（这条路由零权限协作者也能调，而单次检测要吃几百毫秒 CPU 与几百 MB 内存） | — |
...
- **隐写检测的上传字节另有 8MP 上限**（`MAX_STEGO_DETECT_PIXELS`）：检测要把整图解码成 raw RGBA 再逐像素比对，实测 36MP → 401ms、RSS 477MB ⇒ 3 个并发就约 1.4GB，常见的 1–2GB 容器直接 OOMKilled；而这条路由在 `publicRoutes` 里（零权限协作者可调，是有意为之：图片管理页要给协作者用），全局桶 600/分钟对它太宽（= 每分钟 240 秒 CPU）。⚠️ 8MP 只作用于**上传来的字节**；按 sign 检测走的是图床里已存在的文件（上传时已经付过代价），那条路**不套 8MP**，否则"验一张自己库里的 20MP 图"会莫名失败。真要验更大的图，先上传到图床再按 sign 检测。
