...
- 设置文章为隐藏
- 在后台 `站点配置/布局设置` 里把 **「是否允许通过 URL 打开隐藏的文章」** 改成「允许」
- 在后台自定义导航栏中添加这篇文章
...
::: tip 保存时报「必须指明要改哪一页」是怎么回事（2026-09 起）

改动接口现在**必须**带标识符：改一个页面要带 `_id` 或 `path`，删一个页面要带 `path`，否则返回 400。

之所以加这条：以前不带标识符的请求会**静默改掉或删除另一个页面** —— 过滤器里那个字段是 `undefined`
时会被数据库丢掉，于是「定位这一行」退化成「随便拿第一行」。这不是理论问题：后台代码编辑器在
**取数据失败之后按 Ctrl+S**，送出的就是一个不带标识符的请求，于是你可能在毫无提示的情况下改掉了
**另一个**页面的内容（自定义页面是公开渲染的原始 HTML）。现在这种情况是一条能照做的报错。

如果你是直接调接口做集成，请在 `PUT` 的 body 里带 `_id`（列表接口返回的那个）或 `path`，
在 `DELETE` 里带 `path`。
:::
...
一键脚本部署的 VanBlog 是 Docker Compose（VanBlog + Mongo），内置 Caddy 占用 80/443
（TCP，外加给 HTTP/3 用的 **443/udp**）。它**不会**变成可以随便再塞项目的 PaaS。旁挂其他站点可以：
