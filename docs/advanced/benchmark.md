...
| 运行时 | Node **v24.21.0** / Alpine 3.24、express 4.22.3、@nestjs/core 10.4.22、mongoose 8.24.4、sharp 0.35.4、Next 14.2.35 ⚠️ **本报告采集当时的版本**；`next` 此后已升到 **15.5.25**，升级后的新基线见 **§2.1** |
...
### 2.1 追加：`next` 15 + `markdown-it` 14 之后的新基线（2026-09-21）

⚠️ 上面 §1/§2 的数字是**依赖升级之前**测的。升级（`next` 14.2.35 → **15.5.25**、`markdown-it` 13.0.2 → **14.3.2**、
`katex` → **`@mdit/plugin-katex` 1.0.1**）之后，在**镜像 `r29-mainline`**（`node v24.21.0` / `next-server v15.5.25`）
的栈上重测了一组，作为**新的对照基线**：

**单请求延迟 p50**（每个路径 60 次，**480/480 成功、0 失败**）：

| 路径 | p50 |
| --- | --- |
| `/` 首页 | **8 ms** |
| 文章详情页 | **8 ms** |
| `/api/public/meta` | **3 ms** |
| 公开文章列表 | **12 ms** |
| `/rss/feed.xml` | **4 ms** |
| `/sitemap.xml` | **3 ms** |
| 后台入口 | **1 ms** |
...
**页面重量**（HTML 原始 / gzip，以及 `__NEXT_DATA__` 占比）：

| 页面 | 原始 | gzip | `__NEXT_DATA__` 占比 |
| --- | --- | --- | --- |
| `/` 首页 | 96,750 B | 22,697 B | 11–20% |
| 文章详情页 | 84,670 B | 26,583 B | 同上区间 |
| `/timeline` | 101,599 B | 21,926 B | 同上区间 |
| `/link` | 38,512 B | 13,555 B | 同上区间 |
| `/tag` | 36,815 B | 12,707 B | 同上区间 |
| `/page/2` | 96,500 B | 21,442 B | 同上区间 |

🔴 **升级本身对首屏 JS 的影响，是用「同一份代码分别构建两个版本」A/B 量出来的**（不是拿本文件里的旧数字当基线 ——
那些数字描述的是**当时的代码**）：14 条共同路由的 First Load JS **每条只涨 2.0–4.0 kB，平均 +2.74 kB**，
且涨幅**均匀**（连共享的 `/_app` 也只 +2.1 kB）⇒ 归因是 **next 15 运行时/共享 chunk 的固定开销**，
不是某个页面被塞了新东西。HTML 每页约 **+350–400 B** 同源。

👉 **由此得出一条方法论**：**A/B 性能对比必须用同一份代码分别构建两个版本**。文档里记着的旧数字**不能**当"改前"那一侧，
否则会把此后所有的代码增长都算到被测对象头上（本轮就差点把"首页 363 kB"误判成 next 15 加了 70 kB ——
同一份代码下 next 14 实测本来就是 **360 kB**）。
...
::: warning 🔴 2026-09-21 更正：失败源已归因到 rootlessport，而"静态 vs API"的对比是跑序造成的假象

本节把结果写成"静态路径 `200=10000 失败=0`、API 路径 `200=9731 失败=269`"，暗示两条路径有本质差别。**实测不是。**

- 🔴 **失败源是 `rootlessport`**（rootless podman 的用户态端口转发器），不是 VanBlog：失败的请求**根本没到达 caddy**
  —— 客户端报 `失败=399` 时，caddy 访问日志里恰好有 **9601** 条 `/api/public/meta`（全 200、全 8272 字节），逐次精确相等；
  另一次客户端报 `失败=1768`、caddy 恰好 **8232** 条。而失败期间 caddy 的 `duration` p50 只有 **0.127s**（max 1.609s），
  服务端应用日志 **0 条 ERROR/5xx**，容器 netns 的 `ListenOverflows`/`ListenDrops`/`TCPReqQFullDrop`/`TCPSynRetrans`
  **全 Δ=0**（1 秒粒度采样跨 86 秒，其中就包含那次 1768 失败）。
- 🔴 **绕过它就完全没问题**：把同一个压测器拷进容器、直连 `127.0.0.1:80` ⇒ **7 次全部 `10000/0`，合计 70,000 请求 0 失败**，
  其中 4 次是紧接在外部路径连续失败之后跑的。而经 rootlessport 的 **15 次里 8 次失败**（399 到 5732 不等，高度可变、非确定性）。
- 🔴 **这条后来被放大到 22 次跑复核，并且逐条对账**（2026-09-21）：容器内 loopback **22 次全部 `200=10000 失败=0`**，
  caddy 访问日志里 `remote_ip=127.0.0.1` 的条目 = **180,000 条，恰好 = 18 次 × 10000，丢失 0、非 200 = 0**；
  而经 rootlessport 的那一侧，客户端累计成功 **151,065**、caddy 实收 **151,076**（= 151,065 + 11 条前置探测，逐条对上）
  ⇒ **caddy 收到的每一条都返回 200**，而 **180,000 − 151,065 = 28,935 条（16.08%）从未到达服务端**。
  ⇒ 🔴 **结论应当这样读：VanBlog 服务端本身在 1 万并发下没有可观测瓶颈**（两侧应用日志 3405 / 1773 行里
  `ERROR|5xx|dial tcp|i/o timeout|connection reset|no available peer|caddy process exited` **全 0 命中**，
  容器 netns 计数器全 Δ=0、`RestartCount=0`、`OOMKilled=false`），瓶颈完全在压测链路的端口转发器上。
  ⚠️ 还有一条同时间窗对照排除了"这段时间机器状态差"：就在 pasta 侧连续失败之后，**同一套栈**的容器内 loopback
  **4/4 全 `10000/0`、宿主 `ListenOverflows` Δ=0**。
- ⚠️ **"静态路径没问题"是跑序造成的假象**：`measure.sh` 第 5 节总是**先跑静态、后跑 API**，所以静态总在
  rootlessport 还干净时跑。让静态不在首跑，它**同样失败**（实测 481、1287 两次）。
  ⇒ 正确表述是：**两条路径在 rootlessport 干净时都是 `10000/0`；重复 burst 之后两条都会间歇失败。**

🔴 **因此 C10K 的前提里必须加一条比内核参数更强的前提：端口发布方式。**
在 **rootless podman + 发布端口**（`docker-compose` 的 `ports:`，也就是本仓库模板的默认形态）下，
**rootlessport 就在数据路径上**，1 万并发的瓶颈是它而不是 VanBlog。要达到并**持续保持** C10K，请任选其一：

| 方案 | 是否可行 | 说明 |
| --- | --- | --- |
| `network_mode: host` | 🔴 **rootless 下不可行（已实测）** | 容器直接用宿主网络栈，理论上没有用户态端口转发；但 rootless 下绑 `:80`/`:443` 都是 **EACCES**（`net.ipv4.ip_unprivileged_port_start=1024`，而高位端口绑定正常），且本镜像的 caddy 监听端口在 `caddyTemplate.json` 里**写死**（`srv0=[":443"]`、`srv1=[":80"]`），**没有任何环境变量能改它** ⇒ 不改镜像/模板就用不了。⚠️ 另外 host 网络下 app 会直接占用宿主的 3000/3001，与宿主上其它服务冲突 |
| root 的 podman / docker | ⚠️ **理论可行，本机未测** | 走 iptables DNAT，是内核态转发，没有用户态代理进程。本机无 root、sudo 需密码，**如实报为未测** |
| 宿主上放真正的反向代理，直连容器 IP | 🔴 **rootless 下不可行（已实测）** | 宿主直连容器的 bridge IP **20 秒超时**，`podman unshare` 里同样超时（netavark 的 bridge 在另一个 netns）⇒ 这条建议**隐含假设了 root podman/docker**，在 rootless 下不成立 |
| `--network pasta`（podman 较新的用户态网络） | 🔴 **实测更糟，不要当缓解** | 16 次跑 **12 次失败**（rootlessport 那侧是 18 次里 10 次），分类全是 `request_err_CLIENT_TIMEOUT` 且每次**恰好 ~30.3s**（客户端超时上限）⇒ 是**请求停滞**不是被干净关闭。机制由内核计数器钉死：宿主 `TcpExtListenOverflows` 逐次暴涨（46299→65967→89264→122422→141755，每次 Δ 约 2–3.3 万）、`ListenDrops` 同步、`SynRetrans` 每次 Δ 5–6k，而**容器侧计数器恒 0** ⇒ 溢出发生在 **pasta 在宿主的监听 socket**（实测 `LISTEN 0 128` ⇒ **backlog 只有 128**），caddy 那侧根本没排队 |

⚠️ 上面这张表的"是否可行"是**实测结论**（2026-09-21），不是推理：`network_mode: host` 与"宿主反代直连容器 IP"
两条都是先按建议做、然后失败，才如实标注的。⚠️ **不要把这张表当成"照做即可"** —— 在 **rootless podman 单机**上，
目前**没有任何现成的部署形态**能真正把转发器移出数据路径；要拿到那一列成绩，需要 **root 的 podman/docker**，
或者**改镜像让 caddy 监听高位端口**（这两条本机都做不了，**未实测**）。⚠️ 另外 rootlessport **内部为什么关连接没能定死**
（它是闭源二进制、无日志、无法插桩），只拿到三条相关量：fd 峰值 70,012（跑后回落 ⇒ 是延迟回收不是泄漏）、
瞬时 CPU 峰值 195%、宿主 ESTAB 峰值 20,060（每条代理连接它两头各持一个 socket）。

⚠️ 复现本节时还有两个会骗人的地方：读 `/proc/net/netstat` 要用 `nstat -az`（那个文件是**键一行、值下一行**的两行格式，
直接 grep 会**恒读为空**）；压测期间**不要在容器里跑每秒采样的后台脚本**（实测它会把压测干扰到"输出全空、
ESTAB 峰值只有 53"，看起来像服务端挂了，其实是采样器自己污染了被测系统）。看 burst 期的 CPU 要用
`/proc/<pid>/stat` 的 utime+stime **逐秒差分**，`ps -o pcpu` 给的是**生命周期均值**。
:::
...
### 5.4 🔴 C10K 的数字**必须标注 `VANBLOG_CLUSTER_WORKERS` 的取值**，否则两组数字不可比（2026-09-21）

本轮一开始就踩了这条：**同一个镜像、同一套压测工具，只因为 cluster 取值不同，得到了与 §5.2/§5.3 结论相反的结果。**
所以从现在起，**任何 C10K 数字都必须写明 cluster 取值**，否则它没有可比性。

| 口径 | 静态路径（caddy 直服） | API 路径（caddy → Node） | 失败类 | 容器 netns `ListenOverflows` |
| --- | --- | --- | --- | --- |
| **默认**（未设置 ⇒ **单进程 Node**） | **10000/0** | **8221–8952 / 10000**（失败 1048–1779） | 🔴 **`http_502`** | 🔴 **大幅上涨**（实测 Δ 达 **25224 / 16881 / 19415**） |
| **`VANBLOG_CLUSTER_WORKERS=auto`**（本机 6 核 ⇒ 1 主 + 6 worker） | **10000/0** | **6 臂里 5 臂 `10000/0`**（含两个发布端口臂） | 唯一失败臂是 **`CLOSED_NO_RESPONSE` + `ECONNRESET`** | 🔴 **所有臂 Δ=0** |

⚠️ **代价要一起写**：**cluster 会把内存乘以 worker 数**，所以它**默认是关的**（`main.ts` 里解析 `VANBLOG_CLUSTER_WORKERS` 那段上方的注释早已记载，与 AGENTS §7.44 一致）。
〔2026-09-22 更正：这里原写的是行号 `main.ts:612-618`，而该行号已漂移（注释现在在 `VANBLOG_CLUSTER_WORKERS` 解析处上方，约 `:631`）。
👉 **文档引用代码位置时请写"文件 + 符号名"而不是行号** —— 行号必然随改动漂移，而漂移后的行号看起来仍然"像个引用"，不会有任何守卫报红。〕
🔴 **所以"单进程口径下 API 路径只有 8221–8952"不是回归，而是默认配置的既有天花板。**

👉 **给部署者的建议**：**多核机器上要万级并发，就把 `VANBLOG_CLUSTER_WORKERS=auto` 当成部署前提**（并预留 worker 数倍的内存）。

#### 🔴 两种失败机制的判据（这张表比"看失败数"有用得多）

只看"失败了多少个"会得出完全错误的结论 —— **同样的失败数，两种成因的处置相反**：

| 观察到的形状 | 成因 | 处置 |
| --- | --- | --- |
| **`http_502`** 且容器 netns **`ListenOverflows` 上涨** | 🔴 **服务端 accept 队列溢出**（单进程天花板；backlog 4096 对 1 万条连接必然溢出） | **开 cluster**（`VANBLOG_CLUSTER_WORKERS=auto`）；再不够就加 backlog 与核数 |
| **`CLOSED_NO_RESPONSE`** / **`ECONNRESET`** 且 **Δ=0** | 🔴 **请求根本没到达容器** = rootless podman **发布端口**形态下的 **rootlessport** 瓶颈 | **不是 VanBlog 的问题**：改用容器网络内部口径测，或换 root 的 podman/docker、或换 rootful 网络（见 §5.3 那张已实测否掉的缓解手段表） |

⚠️ **判据的关键是"内核计数器有没有动"**：`ListenOverflows Δ=0` 而客户端报了大量失败 ⇒ **失败发生在到达容器之前**。
这也解释了 §5.3 那次"客户端报 `failures=399` 而 caddy 只记录了 9601 条"的现象。

#### ⚠️ 本轮数字的可比性限制（如实标注，别当成受控对比）

- **Arm B/C 是 6 臂背靠背跑的、中间没有冷却**，收尾时 1 分钟负载达 **14.66–15.64**（本机 6 核）⇒
  **后面的臂比前面的臂更疲劳**（这正是本文件里既有的"首跑干净、后续退化"现象）。
  🔴 **唯一失败的那一臂恰好是最后一臂**，所以它的失败**无法与"机器疲劳"完全分离**。
- ⚠️ 相比之下 **Arm A 之前核实过宿主是安静的**（load 0.67 / `nproc`=6）。
- 👉 **要复现请分节跑、每节之间等负载降下来**（判据：1 分钟 load 明显低于 `nproc`）。
- ⚠️ **§6（容器资源占用）本轮只拿到"空闲"那一半**：`vb-c10k-app` **CPU 19.27% / MEM 1.072 GB**；
  **"加压 30 秒"那一半被打断了** ⇒ **引用 §6 时请标明"本轮只有空闲值"**。
...
3. 🔴 **默认限流是 `VANBLOG_RATE_LIMIT_PER_MIN=600`（每 IP 每分钟，clamp 上限 1e6）** ⇒
   **默认配置下，并发 ≥200 的混合流量里大约一半请求会拿到 429**。本轮实测（默认限流、未抬）：
   ...
   ⚠️ **那是限流器在工作，不是站点扛不住** —— 同一轮里**静态直服那一节 800/800 全 200、1393.7 rps、0 个 429**
   （静态走独立且宽 10 倍的桶）。👉 **看到成片 429 先确认限流配置，再谈容量。**
4. 🔴 **容器健康探测：用 compose 部署，或者自己加 healthcheck。**
   `podman`/`buildah` 以 **OCI 格式**构建时会**丢掉 Dockerfile 里的 `HEALTHCHECK` 指令**
   （构建日志里是一条 warning：`HEALTHCHECK is not supported for OCI image format and will be ignored`；
   镜像的 `Config.Healthcheck` 字段在 OCI config 类型里根本不存在，`podman ps` 也没有健康列）。
   ⚠️ **但 compose 模板自己带了一份 healthcheck**，而且有守卫钉住"两处逐字节相同" ⇒
   **按 compose 部署（本文档与安装文档的主路径）是有健康探测的**；
   🔴 **裸 `podman run` / `docker run` 则没有** —— 此时 `restart: always` **不会因为 unhealthy 而重启**，
   于是"容器 Up 但站点已经坏了"没人会发现。
...
2. **打开 cluster**（`VANBLOG_CLUSTER_WORKERS`，默认 1）：代码与守卫都铺好了（cron / 子进程 spawn / 启动清洗 /
   首轮渲染都限定主实例，限流与连接池按 worker 数摊薄）。🔴 **2026-09-21 更正：`N>1` 已经真跑过了**，
...
::: warning 2026-09-21 标注：上面这段"仓库状态"已经是当时的快照，不要当成今天的
`next` 已升到 **15.5.25**（React 仍是 18.2.0），`markdown-it` 升到 **14.3.2**、`katex` 迁到 **`@mdit/plugin-katex` 1.0.1**；
升级后重新构建的镜像是 **894 MB**（比 `v2026.9.2` 的 871 MB 多 23 MB，方向与 next 15 运行时变大一致），
镜像内实测 `node -v` = **v24.21.0**、前台跑的是 **next-server v15.5.25**。
🔴 **本报告正文各节的数字仍然是 Next 14 那一轮采集的**，升级后的延迟 / 页面重量 / C10K 口径见 **§2.1** 与 **§5.4**。
👉 方法论照旧：**A/B 对比要用同一份代码分别构建两个版本**，不要拿这里记着的旧数字当"改前"那一侧。
:::
