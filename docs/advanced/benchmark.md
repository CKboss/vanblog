...
| 方案 | 说明 |
| --- | --- |
| `network_mode: host` | 容器直接用宿主网络栈，没有用户态端口转发（⚠️ 会失去端口隔离，且要与宿主上其它服务避开端口冲突） |
| root 的 podman / docker | 走 iptables DNAT，是内核态转发，没有用户态代理进程 |
| 宿主上放真正的反向代理 | 容器用 host 网络、或由反代直连容器 IP，避免"发布端口"这条路径 |

⚠️ 这三种缓解**本轮都没有实测**，是按"绕开 rootlessport 后 7/7 全 0 失败"这个实测结果推出来的方向；
要确认请在你自己的部署形态上复跑本节协议。⚠️ 另外 rootlessport **内部为什么关连接没能定死**
