...
> 下面是 `v2026.9.6` 打标之后累积的改动，还没发版。

### 2026-09-25：多语言 —— 后台「系统设置」的图床与评论页签跟随语言

- ✅ **`系统设置 → 图床设置` 与 `系统设置 → 评论设置`（Waline 卡片）现在跟随站点语言**（简中 / 繁中 / English）：
  卡片标题、按钮、表格列名、提示与确认弹窗全部翻译，共 **31 条新文案**，语言包 **114 → 145 key**。
- 🔴 **英文的计数文案用了正确的复数**：「扫描成功！共 N 项」在英文下是 `1 item` / `2 items`（ICU `plural`），
  「改写完成：文章 N 篇，草稿 N 篇，共 N 处」三个计数各自变复数 ⇒ 不会再出现 "1 items"。
- 🔴 **繁体中文用地区用词**（圖床 / 儲存 / 進階 / 網域 / 壓縮檔 / 匯出 / 檔案 / 資料庫 / 外部連結），不是字形转换。
- ✅ **防回归**：这两个文件已纳入「硬编码中文棘轮」且**预算为 0** ⇒ 今后在其中新增硬编码中文会立刻测试失败；
  棘轮覆盖的文件从 9 个增加到 11 个，而**允许的硬编码中文总量仍是 48 条**（覆盖面扩大、总量不增）。
- 🔴 **顺带修掉一个繁中译文缺陷**：「掃描现有文章圖片到圖床」里的 `现` 是简体（应为 `現有`），
  已修正，并把该字补进「zh-TW 不许出现简体专用字」的守卫字表（附变异对照证明其承重）。
- ⚠️ **已知边界（刻意保留，不是漏翻）**：
  - 🔴 **切到 English 后，「系统设置」页顶部的页签标签仍是中文**（站点配置 / 主题 / 定制化 / …）。
    原因：这些标签名同时被**后台导航路径提示**（`站点管理 / 系统设置 / …`）与**文档**引用，
    而文档多语言尚未决定 ⇒ 只翻页签会造成"界面英文、路径提示与文档中文"的对不上。
    等文档口径确定后一批做完。
  - 后台其余页面（文章、编辑器正文、数据管理、日志、用户、Token、HTTPS、高级设置、备份恢复、主题、迁移助手）
    与「站点配置」表单仍是中文，按批次推进。
  - 前台（访客站点）本期未做多语言。
