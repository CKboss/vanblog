...
| 界面风格 | Apple 风格 | 前台展示页的视觉风格：`Apple 风格（推荐）`（单列发丝线列表、SF Pro 字体、17px 正文、毛玻璃导航、苹果蓝强调色，自动适配暗色）或 `默认（原卡片风格）`。这里还会列出你在 **系统设置 → 主题** 里上传的自定义主题（上传与管理在那个页签）。只改样式不改结构，随时可切回，见 [主题](../features/theme.md)。 |
