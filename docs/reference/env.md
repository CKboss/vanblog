...
| `VANBLOG_CLUSTER_WORKERS` | `1`（代码默认）／🔴 `auto`（**镜像默认**，Dockerfile 里设的） | 多进程（cluster）worker 数：正整数（上限 **32**），也可写 `auto` / `cpus` / `max` = 按 CPU 核数开（同样不超过 32）。空值、`0`、负数、垃圾值一律回落到 `1`（= 单进程）。⚠️ N>1 时：内存近似线性增长（每个 worker 一份完整应用）；限流与连接池会按 worker 数摊薄；初始化 / 整站恢复由数据库里的 [TTL 锁](../advanced/security.md#认证)互斥（抢不到锁的一方得到 **409**）。打开前请自己压一遍。<br>⚠️ **停机语义**：关停时若有 worker 在宽限期内没退出会被 SIGKILL，此时主进程以 **ExitCode 1** 退出并打一条 FATAL（本轮之前是退出 **0**，编排系统看不出异常）。⚠️ 这个宽限期是 **cluster 主进程等 worker** 的时间，写死在代码里（10000ms）、**没有**对应的环境变量；它与下面那行 `VAN_BLOG_SHUTDOWN_TIMEOUT_MS`（默认 8000，管的是**容器 start.js 等子进程**）是**两个不同层的超时**，别混。正常的 `docker stop` / `compose down` 仍退出 0，`restart: always` 也不会因此循环重启（重启策略不作用于显式 stop）。<br>🔴 **N>1 时必须有一个 worker 是「主实例」**，否则前台子进程、初始化密钥、定时备份、启动数据清洗等"只能跑一次"的活会全部静默跳过（详见下面[内部变量](#内部变量-不是对外契约别设)里 `VANBLOG_CLUSTER_ROLE` 那一段）。`ea9547f3` 之前正是这个形状：集群模式下前台全 502、既不能初始化也不能用归档恢复 |
| `VANBLOG_DISABLE_WEBSITE` | 空 | `true` = server 不拉起前台（Next）子进程。前后端分离部署 website 镜像时用。🔴 **注意**：设了它之后 `/api/public/health` 的 `website` 字段会报 `disabled`（这**不是故障**，状态码也不会因此变成 503），但也就意味着**这个字段保护不到你的前台** —— server 根本不拉起前台，自然无从判断它是否存活。**前后端分离部署请直接探测你自己的 website 容器**，不要依赖这个字段 |
| `VANBLOG_WEBSITE_HOST` | `0.0.0.0` | 前台子进程的监听地址（一体式镜像保持默认即可） |
| `VANBLOG_WEBSITE_SLOW_RETRY_MS` | `300000`（5 分钟） | 前台（Next）子进程**连续崩溃 5 次**、快速退避阶梯用尽之后的重试间隔，夹在 **60000–3600000**（1 分钟到 1 小时）。空值、`0`、负数、非数字、`NaN`、`Infinity` 一律回落到默认值 ⇒ **任何输入都产生不出 0 间隔**（那会变成紧密重启风暴，比放弃更糟）。⚠️ 改动前的行为是**打一条 ERROR 就永久放弃**：一次瞬态原因导致的崩溃风暴之后（端口被残留进程占住、磁盘临时满、数据库短暂不可达），前台会永久不再自动拉起，即使那个原因早已消失，而 server 还活着 ⇒ `restart: always` 不介入，用户只看到页面 502，需要人工重启容器。现在改成**长间隔的无限退避**，瞬态原因消失后能自愈；子进程稳定跑过 60 秒即恢复完整的快速阶梯，手动重启（后台保存站点信息等）也会把退避状态整个清掉 |
| `VANBLOG_WALINE_SLOW_RETRY_MS` | `300000`（5 分钟） | 同上，但作用于 **waline 评论服务**子进程，夹在 **60000–3600000**，回落规则完全一致。⚠️ **刻意不与上面那个共用一个变量名**：两者的严重度不同（前台坏死 = 整站 502，waline 坏死 = 只有评论不可用，`/comment*` 与 `/ui` 502 而站点其余部分正常），站长应当能分别调；默认值与范围故意一致，因为取舍理由相同（瞬态原因通常几分钟内消失，而一小时以上的间隔等于放弃自愈）。⚠️ waline 坏死的症状比前台坏死**更难被发现**：server 还活着、页面照常打开，用户只会反馈"评论发不出去" |
...
| `VANBLOG_MONGO_READY_TIMEOUT_MS` | `60000` | 启动时「等数据库就绪，再触发全量重渲染」的上限（毫秒），夹在 0–3600000。⚠️ **超时不影响启动** —— 站点照常起来、继续对外提供已缓存内容，只是这一轮预热很可能失败并退化成按需渲染，日志会打一条 WARN 说清后果与兜底。⚠️ **写 `0` 得到的是默认 60 秒，不是「不等」**（`envPositiveInt` 的既定规则：缺失/空/非数字/≤0 一律回落默认）；想几乎不等请写 `1`。<br>为什么要等：改动前重试窗口只有约 18 秒，而容器重启后数据库重连常常超过它，于是启动预热被**永久放弃**，只能等每小时的定时重渲染 |
| `VANBLOG_MONGO_READY_POLL_MS` | `1000` | 上面那个等待的轮询间隔（毫秒），夹在 50–60000。就绪判据是 `readyState === 1` **并且**一次真实的 `ping()` 成功，与健康检查接口同一口径，所以不会出现「健康检查说 503 而这里说就绪」 |
| `VANBLOG_ISR_RETRY_MAX` | `8` | 触发全量/增量重渲染时，最多探活尝试多少次，夹在 1–100。写错值回落默认 |
| `VANBLOG_ISR_RETRY_BASE_DELAY_MS` | `3000` | 首次重试的间隔（毫秒），之后每次翻倍，夹在 100–600000 |
| `VANBLOG_ISR_RETRY_MAX_DELAY_MS` | `30000` | 重试间隔的封顶（毫秒），夹在 100–3600000。默认组合（8 次、3s 起翻倍、30s 封顶）的纯等待窗口约 **135 秒**，改动前约 18 秒 |
...
| `VANBLOG_PUBLIC_LIST_LIMIT_PER_MIN` | `60` | 每 IP 每分钟对 `/api/public/category` 与 `/api/public/tag` 的上限。这两个匿名端点**没有分页**（一次返回全部分类/标签及其文章），每次都要全表捞取再分组，所以单列一档，比全局的 600 严 10 倍。夹在 `1`–`100000`；**缺失/非法/≤0 都落回默认值**，所以任何输入都不会变成"不限"。⚠️ 命中这一档之后**仍计入**全局兜底桶，两者取最严的那个（不是二选一）。⚠️ 大小写与尾斜杠变体（`/API/public/category`、`/api/public/category/`）会归一化到同一档——Express 的路由本来就大小写不敏感，不归一化就能绕过。本站内部调用（前台 SSR、waline、ISR 触发）豁免，否则站点会把自己 429。被 429 时响应里会指出替代方案：`/api/public/article?category=…&page=…&pageSize=…` 是**数据库级真分页** |
| `VANBLOG_PUBLIC_LIST_CACHE_MS` | `5000` | 上面那两个端点的结果缓存 TTL（毫秒），**上限 60 秒**（超过会被夹到 60000）。⚠️ **`0` 是合法值，表示"不缓存"**（与上面那个限流变量不同：限流变量 `≤0` 会落回默认值，而这里只有**负数与非数字**才落回 5000）。`/api/public/tag/<标签名>` 复用**同一份**缓存条目，不按标签名建键——标签名是访问者可控的，按名建键等于开一个无界键空间。⚠️ **行为影响**：发文/改文/删文之后，这两个端点**最多 5 秒**才反映变化；前台本身是 ISR（文章页的再验证是**天**量级），所以这个窗口比你已经体验到的陈旧窗口紧得多。⚠️ 缓存存的是对象而不是序列化后的字符串，所以每个请求仍要 `JSON.stringify` 一遍——文章极多时这部分 CPU 仍随文章数增长，由上面那一档限流兜住 |
...
| `VANBLOG_SANITIZE_MAX_NODES` | `50000` | 单次请求净化时最多允许访问多少个 JSON 节点（对象/数组/键值各算一个），夹在 1000–5000000，写错值回落默认。⚠️ **`0` 不是关闭** —— 它是安全边界，没有关闭档位，写 `0` 会被夹到 1000。超限时**拒绝请求（413）而不是跳过净化**：跳过就等于把 `$` 操作符与 `__proto__` 原样放过去。⚠️ **口径别搞错**：数的是**节点**（键与值分别计数），所以 50000 节点大约等于 **25000 个键** —— 实测拐点是 24000 个键仍通过、30000 个键被 413。为什么 50000 够用：本站的大 body 大在**字符串值**上（正文内嵌 base64、整页 HTML），而净化只递归对象与数组、字符串原样返回，所以 50MB 的合法正文只花几十个节点。效果是把最坏情况的净化成本从「50MB ≈ 7.5 秒同步阻塞事件循环」压到约 **33 毫秒** |
