...
站点开启 **Apple 风格**（`站点信息 → 布局设置 → 界面风格`）时，编辑器的**预览面板**会用与前台
...
点击右上角操作按钮，选择 `修改信息`，即可对当前编辑的 `文章/草稿` 的信息就行修改。标题、「自定义路径名 / slug」等输入框可用左右方向键移动光标，也可用退格键 / Delete 删字。从 Hugo 迁过来时，在「自定义路径名 / slug」填旧文章的 slug，发布地址即为 `/post/<slug>`（留空则用数字 id）。标签框支持一次粘贴多个标签（逗号 / 分号 / 换行分隔）。文章还可以在这里设置**定时发布**（`publishAt`，到点前所有公开面不可见、到点自动上线，见 [文章管理 → 定时发布](./article.md#定时发布publishat)）；访问密码框的语义是「**留空 = 不修改**」，解除加密要显式勾「清除密码」（密码只存哈希、不可找回，见 [加密文章](../advanced/encrypt.md)）。
