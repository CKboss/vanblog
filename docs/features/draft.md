...
你可以点击 `导入` 按钮导入 Markdown 文件来创建草稿；**`.mdz`（Typora 图片包）走编辑器的
「导入内容」**（选择器接受 `.md,.mdz`，图片自动入图床、相对链接改写成图床 URL，
填进编辑器后由你审阅保存；Front Matter 里的 `password` 会被丢弃），见 [文章管理 → 导入文章](./article.md#导入文章)。
...
| top | 置顶优先级 | 默认为 0 |
| cover / image | 封面图 | 只认非空字符串（两个名字都认） |
| pathname / slug / url | 自定义路径 | 优先级就是这个顺序；`url` 只认单段或 `/post/<slug>` 形式。留空则按标题生成拼音路径。Hugo 的 `permalink` **不认** |

::: tip 导入只认 `.md`

「导入」按钮只接受 `.md` 文件（其它扩展名会直接提示不支持）。
`.mdz`（Typora 图片包）请走编辑器的「导入内容」，见上面那段。
...
发布之后到 `文章管理` 的「修改信息」里设置**定时发布**（`publishAt`）：到点前文章在所有
公开面都不可见，到点自动上线，详见 [文章管理 → 定时发布](./article.md#定时发布publishat)。
...
导出时会自动映射信息到 Markdown 文件的 FrontMatter 中。导出同样是 `.md` / `.mdz` / `.zip`
三选一，见 [文章管理 → 导出文章](./article.md#导出文章)。

也可以**批量导出**：多选后点「批量导出」。⚠️ 批量导出走的是**另一条路**（在浏览器里逐篇转成 Markdown 直接下载），
每篇得到一个裸 `.md`（带 Front Matter），**不含图片、也不能选格式** —— 要图片包请逐篇用上面的「导出」。

## 删除草稿与回收站

删除草稿是**软删除进回收站**：`草稿管理 → 回收站` 里可以随时**恢复**；
只有回收站里的**彻底删除**（显式二次确认）才不可撤销。
