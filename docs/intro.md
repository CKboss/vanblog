...
- [x] 快到极致的响应速度：静态化前台 + ISR 增量渲染。实测首页 gzip **22,961 B / 8–10 ms**、混合流量 **456–506 rps**（0 个 5xx），复现命令见 [性能实测](./advanced/benchmark.md)。
...
- [x] 版本号展示（前台页脚与后台「关于」）和更新提醒 —— 只在**两边都是正式发布号**时才按数字段比较，源码构建的 `dev/dsh@<sha>` 不会弹假警报。
- [x] 内置强大的分析功能，可统计访客等数据。并配有精美看板。
- [x] 内嵌评论系统（存在本站的 Mongo 里，不必外挂；也可改用 Waline）。
- [x] 强大的 Markdown 编辑器，支持图表和数学公式，一键插入 more 标记，一键剪切板及本地图片上传，支持自定义高亮块语法，支持 Emoji 表情选取。
- [x] TOC、草稿、代码复制、访客数、评论数、分类、标签、加密、友链、打赏、自定义导航栏。
- [x] 站内搜索：构建期生成静态索引（零数据库成本、可被 CDN 缓存）+ `/search` 结果页（排序、高亮、分页），索引不可用时自动回退服务端搜索。
- [x] 文章回收站、历史版本（回滚前会先存一份当前状态，所以回滚本身可回滚）、定时发布、阅读时长与相关文章推荐。
- [x] 多个布局设置，可自定义页面细节。
- [x] 高度定制化，可添加自定义 CSS、HTML 和 JS 代码。
- [x] 支持自定义页面。
- [x] 前台皮肤可换：后台上传一份 CSS 就是一个新主题（主题文件随整站备份一起走）。
- [x] 可添加具有指定权限的协作者。
- [x] 内置图床，并支持各种 OSS 图床、github 图床（外部图床基于 picgo）等。
- [x] 支持上传图片自动添加水印，无论何种图床；**中文水印也可以**（渲染在服务端做，镜像自带 Latin 与中文字体）
- [x] 极致轻量化，没有花里胡哨。页面秒切换、图片懒加载。
- [x] 脚本一键部署，多种部署方式。⚠️ 发布的镜像只有 **linux/amd64**；ARM 机器装得了，但走的是**源码构建**（脚本拉不到镜像时自动回退，或显式 `VANBLOG_INSTALL_MODE=source`），要 20–40 分钟。
- [x] 支持 GA、百度分析
- [x] 简单易用的后台，支持数据的导出与导入。
- [x] 整站备份/恢复：导出后立刻自校验、`verify-deep` 语义校验、`drill` 恢复演练（在一次性栈上真恢复一遍并逐项断言）。
- [x] 支持 RSS 订阅
- [x] 完善的 API，完全利用本项目后台和服务端，自己写前端或适配其他页面生成器。⚠️ 运行时的 `/swagger` **默认关闭**，要用设 `VANBLOG_SWAGGER=true`；接口清单另见 [API 参考](./reference/api.md)。
...
- [x] 标签管理
- [x] 黑暗模式图标样式优化
- [ ] 内嵌评论的邮件通知和 webhook（**内置评论目前没有**通知能力，也没有点赞 UI 与验证码；要邮件通知请用可选的 Waline，它自带 SMTP）
- [x] 自定义 css
- [x] 添加自定义 script 标签
- [x] 添加自定义 html 代码
- [x] 可添加具有自定义权限的协作者
...
- [x] 批量操作文章草稿
- [ ] 插件系统
- [x] 自定义主题（**换肤这一层已经做了**：后台上传一份 CSS 就是一个新主题，可多主题共存随时切换）
- [ ] 自定义主题的**渲染器**层（换掉整套前台组件，不只是样式）
- [ ] 浏览器消息通知
- [x] 文章/草稿的历史版本管理（改标题/正文才快照，上限可配；另有回收站与定时发布）
- [ ] 异地备份（S3 / OSS / WebDAV）—— 目前备份只落在本机，是最大的单点
- [ ] 后台 2FA（TOTP）与会话管理（"登出所有设备"）
- [ ] `/metrics`（Prometheus 指标）
- [ ] 响应式图片 `srcset`（现在只有"缩略图 + 原图"两档）
- [ ] 自定义**图片**水印（上传一张 logo 当水印；文字水印已支持中文）
- [ ] 精简配置项，尽可能移动到运行时配置
- [ ] 增加 ORM 层，适配更多数据库
- [x] 增加一些 e2e 测试，集成到 CI（`admin-e2e.yml`：37 个 spec / 111 用例，真浏览器渲染真组件）
- [ ] 国际化
...
> ⚠️ 最后这张 Lighthouse 截图来自上游时期，**本分支没有重测**，不要当成当前成绩。
> 本分支的性能口径是[性能实测](./advanced/benchmark.md)里那组数字（首页 gzip 22,961 B / 8–10 ms、
> 混合流量 456–506 rps、C10K 一万条连接等），采集脚本已入库，每个数字都能重跑对账。
