...
这是**登录防爆破**在起作用，不是密码错了，也不是账号坏了。默认开启：同一个来源 IP 在 300 秒内失败 5 次，之后的登录请求会被直接拒绝，提示变成「错误次数过多！请 N 秒后再试！」（N 是还要等多少秒）。这时候**即使输入正确密码也会被拒**，等倒计时走完再登就行。

实测顺序是这样的：第 1–5 次错密码回 `401 用户名或密码错误！`，第 6 次起回 `401 错误次数过多！请 295 秒后再试！`。登录成功会把计数清零。

有两点容易误会：

- **锁的是登录接口，不是整个后台**。已经登录的会话不受影响 —— 锁定期间用浏览器里原有的登录态照样能进后台、能调接口。所以「登不进去」和「后台还能用」可以同时成立。
- **计数按访客真实 IP 分桶**，不是按服务器看到的地址。所以别人乱试密码通常不会把你锁在门外；但如果你套了 CDN 或隧道，且反代是**覆盖**而不是追加 `X-Forwarded-For`，就要把 `VANBLOG_TRUST_FORWARDED_HEADERS` 设成 `always`，否则同一出口 IP 的人会共用一个桶。

次数与秒数在后台没有表单字段（只能改开关），阈值细节与 IP 判定见 [安全策略](../reference/secure.md)。真的忘了密码走 [忘记密码](./password.md)。
...
不是坏了，多半是**索引还没生成**。搜索用的是一份静态索引 `/static/search/index.json`，它和 RSS、sitemap 走同一条触发链，有 **60 秒防抖**：全新站点刚起来的第一分钟里这个文件还不存在（请求它会得到 404），刚发布或刚改完的文章也要等这一轮跑过才进索引，另外每小时还有一次兜底。

这期间结果页会**自动改用服务端搜索**，并在页面上写明「当前使用服务端搜索：`<原因>`」—— 所以搜索不会变成一块空白，只是多一行提示。等一分钟再搜就正常了。

如果等了还是搜不到，先分清是"没索引"还是"匹配不上"：索引里只有标题、路径、标签、分类和 ≤200 字的纯文本摘要，而且是**子串匹配、不分词、不容错拼写、没有拼音**，关键词藏在正文很深处时就搜不到。这属于设计取舍，见 [搜索](../features/search.md)。

## 新文章没有出现在 RSS 里

RSS 的防抖是 **3 分钟**，比 sitemap 与搜索索引（60 秒）长 —— 所以「已经搜得到、feed 里却还没有」是正常的，不用去查是不是坏了。实测：新建一篇文章后 60 秒内 `static/rss/` 还是空目录、`/feed.xml` 回 404，3 分钟后才出现。

订阅地址是 `/feed.xml`（RSS 2.0）、`/atom.xml`、`/feed.json`（由 caddy 改写到 `/rss/` 下）。另外 feed 默认只带**最新 50 篇**（`VANBLOG_RSS_ITEM_LIMIT`，设 `0` = 全量），`<description>` 是摘要（前 200 字），全文在 `<content:encoded>` 里 —— 阅读器只显示摘要的话是它自己没读全文字段。细节见 [RSS 订阅](../advanced/rss.md)。
...
临时办法：在 **站点管理 / 系统设置 / 高级设置**（标签页，不是站点配置里那个同名的子标签）里手动触发静态页面更新。若直连已是新内容、走反代仍是旧页，见 [上一节](#后台发布后前台不刷新仍显示旧文章)。
