...
这两条路都**不需要你手工去抄下面的初始化密钥**：脚本会自己从 `<数据目录>/log/setup.key`
（读不到再从容器日志的「初始化密钥： 」行）取出来带上，密钥不回显。
...
- 站点**已经**初始化过时，这三个变量被忽略（日志里 INFO 说明一句），不会覆盖已有账号 ——
  改密码请登录后到后台「账号设置」；
- ⚠️ 零接触初始化写进去的作者描述字段是 `authorDesc`。早先误写成 `authDesc`（上游时期的拼写），
  而**没有任何代码读那个键** —— 后台表单、前台类型与所有读取方用的都是 `authorDesc`，
  所以这样初始化出来的站点，作者描述会一直是空的，直到站长恰好在后台保存过一次设置。
  现在字段名已改对，并且**老库会在启动时自动迁移**：只在 `authorDesc` 缺失或空白时把 `authDesc` 的值搬过去，
  搬完把死键删掉；**幂等**（第二次跑匹配不到任何文档），且**绝不覆盖**站长自己填过的内容。
...
- `POST /api/admin/init` 与 `POST /api/admin/init/restore` 都必须携带这把密钥
  （字段名 `setupKey`；脚本/接口调用就放进 JSON 或 multipart 的文本字段）；
- ⚠️ **初始化页的密钥输入框不是一开始就摆在那儿的**：页面加载时**不会**去探测（`/api/admin/init*`
  挂着 5 次/10 分钟的限流桶，用页面加载去探测等于烧掉你自己的提交预算）。流程是：你照常填完向导点提交 →
  服务端回 **400** 且 body 里带 `setupKeyRequired:true` → 页面**这时**才显示「初始化密钥」输入框，
  并把服务端那句指路消息原样展示（大意：本站开启了初始化保护，请在「初始化密钥」一栏填入后重试，
  密钥在日志里反复打印、也写在 `setup.key` 文件里）。所以第一次提交被拒绝是**预期行为**，不是装坏了；
- 密钥文件里是 **44 个字符**的一行 base64（32 字节随机数）。复制时**要完整一行、不要带多余字符**：
  带错了会得到「初始化密钥不正确」的 400，消息同样会指路；
