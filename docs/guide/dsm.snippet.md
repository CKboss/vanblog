...
如果不嫌麻烦，最后把数据存储映射一下（不映射的话，容器一删数据库就没了）：
...
新版**默认开启初始化保护**：站点还没初始化时，匿名初始化接口必须带一个「初始化密钥」，
防止别人抢先把你新装的博客初始化成他的。⚠️ 输入框**不是一开始就有**：你照常填完向导点提交，
第一次会被拒绝并提示需要密钥，**这时**页面上才出现「初始化密钥」一栏 —— 这是预期行为，不是装坏了。
密钥每次启动重新生成，两个地方能拿到：

- 你映射出来的**日志目录**里的 `setup.key` 文件（群晖上用 File Station 打开；按下面
  「存储空间映射」那一步把 `/var/log` 映射出来后，就是 `/你映射的目录/setup.key`）；
...
可以参考 [环境变量](../reference/env.md) 里的目录映射部分进行映射。
