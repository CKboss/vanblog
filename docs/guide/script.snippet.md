...
::: note 2026-09-22 标注：上面那次实测的数字是"当时"的，差值本身仍然自洽

那段话写于工作区脚本还是 **198,281 字节**的时候：`v2026.9.2` 标签上的 `scripts/vanblog.sh` 是 **173,377 字节**
（用只读 git 复核过，今天仍然如此），两者相差 24,904 字节 ≈ 25 KB ⇒ **"少 25 KB"在当时是准确的**。
此后脚本已增长到 **331,150 字节**，而 `v2026.9.3` 的附件也是 **331,150 字节**，
所以"最新附件"与"当前脚本"此刻**恰好一致**。

⚠️ **但这不改变上面那条警告的结论，反而印证了它**：附件是**打标签那一刻的快照**，
"当前一致"只是巧合（正好在发版之后没有再改脚本），而**任何更早的标签都必然落后**
（`v2026.9.2` 就落后 158 KB，缺 `update <版本号>`、也不带初始化密钥）。
👉 所以判断依据不该是"附件比脚本小多少"，而是**"附件会不会随后续修复更新"——它不会**。
:::
