...
渲染「两个半透明光斑 + 对角底色渐变 + 极淡斜纹（`repeating-linear-gradient`，纯 CSS 不引图）」。
**光斑位置也由标题哈希得出**（`coverOrbs()`，两个带种子的哈希分别定位，范围夹在画面内），
所以相邻卡片不只是色相不同、构图也有差别，而同一篇永远稳定。
`ListThumb` 因此改成**总是渲染**：有图用图 → 缩略图失败退原图 → 原图也失败退占位封面。
⚠️ **不要在占位封面上写标题文字**：第一版放了「标题首字」，用户反馈"中文标题取一个字很怪"——
既没语义又像乱码，而且标题就印在封面旁边属于重复。Apple 自己的占位图也是抽象渐变。
（顺带记一下当时踩的坑：用 `split(/\s/)` 取首词，`2025冬日下的天马山` 会得到 `2025冬日下的`
这种半截词 —— 中英混排标题不能用空白分词。）
整块是 `aria-hidden="true"`（纯装饰，读屏不该念）。
