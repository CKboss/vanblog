...
### 7.171 期 6 第六批：备份与恢复页（89 条 → 0）—— 🔴 批量改写工具**当场炸出**一个"构建绿 + 单测绿 + 类型门禁绿、只有浏览器能看见"的白屏缺陷

**交付**：`pages/SystemConfig/tabs/Backup.jsx`(89) → **0**（🔴 本项目单批最大）；
语言包 **930 → 1013 key**（新组 **`backup`**：82 新 + 1 个"拆语境"；**复用 3 条**：`common.colOption` /
`common.delete` / `common.uploadFailedWithName`）；棘轮清单 **77 → 78 个文件**（**TOTAL 仍 61**）；
`i18nKeyNaming` → **1013**；`localePackParity` 自动发现下界 **71 → 72 个文件 / 1210 → 1300 个调用点**（实测 72 / 1311）；
繁中同形白名单 **+4**（`backup.secondsValue`/`colFormat`/`includesPrefix`/`includesAnd` —— 没有简体专用字）。
🔴 **真实剩余：54 → 53 个文件 / 783 → 694 条**。
🔴 **浏览器活体 35/35（zh-CN 11 + en-US 12 + zh-TW 12），problems 0、skipped 7**（每条 skip 都写明原因）：
两张卡片标题、五个按钮、压缩格式下拉、六个表格列头、**行内五个动作**、
以及 🔴 **真的点了一次「导出整站备份」** ⇒ 生成成功弹窗的标题 + Descriptions 六个标签 + 六格内容
（含 ICU 三占位符那格：`32 筆資料 / 0 個檔案 / 16 張表`、`0.01 MB（壓縮前 0.0 MB 靜態檔案）`）。
en-US 实采：`Full-site backup and restore` / `Export full-site backup` / `Upload backup and restore` / `Refresh list` /
`Import all data` / `Export all data` / `Backup file·Size·Format·Contents·Created at·Actions` /
`Download·Sign·Manifest·Restore·Delete` / `Full-site backup created` / `File·Size·Compression·Duration·Contents·Databases`。
zh-TW 实采（地区用词）：`整站備份與還原` / `資料備份（僅資料庫記錄， JSON）` / `匯出整站備份` / `上傳備份並還原` /
`重新整理列表` / `匯入全部資料` / `備份檔案` / `產生時間` / 🔴 `簽章` / 🔴 `還原` / `整站備份已產生`。
外加两条反向判据：en-US **整页零汉字**、zh-TW **零简体专用字**（并且先证明"采到 ≥20 段文字"）。
证据：`vanblog_dev/i18n-browser-evidence/phase6-backup/`。

#### A. 🔴 工具当场炸出一个"处处绿、只有浏览器能看见"的白屏
`const FORMAT_LABELS = { auto: '自动（挑本机最强的）', … }` 是**模块级常量**；codemod 照映射表把它包成了
`t('backup.compressorAuto', …)` ⇒ 🔴 **模块加载期 `t` 不存在** ⇒ 整个路由 chunk 抛 `ReferenceError` ⇒ 页面白屏。
危害在于它**处处绿**：`umi build` rc=0（语法没问题）、`node --test` **全绿**（没有测试渲染那个页面）、
类型门禁 **31/0**（`.jsx` 不参与 tsc）⇒ **只有浏览器活体看得见**（内容区一片空，还伴随一条无关的 401 噪声）。
👉 🔴 这就是"UI 可见改动必须真浏览器验证"这条规矩的价值证明：本批若只看测试矩阵就提交，
上线的是一个**打不开的备份页** —— 而且是最需要它能打开的那一页。
两处修法：① 常量改**函数版** `formatLabels(t = IDENTITY_T)`（本项目既有做法），调用点传 t；
② 🔴 **工具本身学会拒绝**：`wrapTCalls.js` 现在会数"这个字面量外面套了几层函数"，
**depth = 0（模块作用域）一律不改写**，而是单独列出来（行号 + 想用的 key + "手工改成函数版"的提示），
并且 🔴 **"还剩几条"不许把模块作用域那几处算成已清完**（否则工具会打印"✅ 清完了"）。
配套守卫（工具那组第 6 条）+ 变异对照：把"模块作用域也照改"放回去 ⇒ 红。
⚠️ 探针也学了一条：🔴 **选择器超时时先取证再抛**（打印 `body.innerText` + 最近的 console 错误），
否则"页面白屏"和"路由写错"看起来一模一样（本批第一版就误判成路由问题，连着改两次 URL 才发现是白屏）。

#### B. 🔴 英文**接缝双空格**：第 5 次踩，这次终于有了静态守卫
`包含 <b>数据库全部集合</b>（…）、<b>waline 评论库</b>，以及 <b>本地静态文件</b>（…）` 这种"文字 + `<b>` + 文字"
只能拆成 prefix/suffix 多个 key（react-intl 3.x 没有富文本占位符）。起草的英文值自带接缝空格
（`"Includes "` / `", and "`），而 JSX 里 `{t(...)} <b>` **本来就有一个空格** ⇒ 渲染成 `Includes  all database collections`。
🔴 而且是**活体探针采到的文本**才看出来的 —— 静态判据一条都没红（值本身合法、占位符也对得上）。
⇒ 新增守卫：**按页面上的真实顺序把片段组装起来**（`SP` 表示 JSX 里那个字面空格），断言 en-US
不许出现 `  `（双空格）、不许"标点前有空格"、不许"左括号后有空格"；并且 🔴 **zh-CN 的组装结果必须与
改造前页面上的原文逐字相同**（这才是"没偷偷改文案"的证据）。4 条链都进了这条守卫
（"包含 …"那段 / "压缩格式 …归档存在 …"那段 / `将用 X 覆盖：` / `（压缩前 N MB 静态文件）`）。
变异对照 M2 把那个空格加回去 ⇒ 红在"双空格"上。
👉 🔴 **规矩：拆成 prefix/suffix 的英文值，接缝空格只能有一个来源** —— 要么 JSX 里有字面空格、要么值里带，
不能两边都带。**写这类值之前先看 JSX 里那个位置有没有空格。**

#### C. 🔴 同一个中文、两个语境 ⇒ 必须拆成两个 key
`waline 评论库` 在页面上出现两次：一次是恢复确认弹窗里的**列表项**（英文该首字母大写），
一次是"包含 …"那句里的**内联成分**（英文该小写、还带 the）。起草时共用了一个 key 并选了小写版
⇒ 列表项读起来是小写（另两项是大写）。⇒ 拆成 `backup.overwriteWaline`（`The waline comment database`）
与 `backup.includesWaline`（`the waline comment database`），中文两份完全相同。
👉 这与"同值多 key 要收敛"那条待办**不矛盾**：收敛的是**同语境**的重复；
语境不同（大小写 / 冠词 / 单复数会变）就该拆。

#### D. 🔴 委派译文：这次成了，但 brief 必须**内联**约束
第一次委派（25 分钟无产出、已中断）失败在 brief 让它去读 `AGENTS.md`（15k 行）⇒ 上下文被吃光。
第二次改成：**禁止读仓库任何文件**，把 82 条中文、术语表（繁中地区用词 40 余条，全部从既有语言包里取）、
ICU 复数规则、接缝规则、技术标识符清单**全部写在 prompt 里**，只要求写一个文件 ⇒ **约 8 分钟交付**，
质量可用（ICU 复数 8 处全对、术语对齐、无单引号无汉字）。父代理复核抓到 **3 处**：
① 简繁审计红了 1 处（`站点` 里那个简体「点」⇒ 改「站點」）；② B 段的接缝双空格；③ C 段的语境共用。
👉 🔴 **委派译文的规矩**：brief 里**内联**所有约束与术语（别让它自己去读大文件）；
交付后**必须**跑完既有四道闸门（三语 key 集合一致 / defaultMessage↔zh-CN 逐字对账 / 简体专用字审计 /
占位符↔values 对账）+ 活体。🔴 **子代理的自检报告只当线索、不当证据**：
它自称"占位符全部保留"，而父代理第一把尺子因为 ICU 复数的**嵌套花括号**误报了 8 处不一致 ——
两边都可能错，**先把尺子修对再下结论**（本项目"先怀疑尺子"这条又一次生效）。

#### E. 🔴 探针尺子三处错（都被实采数据纠正）
1. **路由**：系统设置是 `/admin/site/setting`（不是 `/admin/system`）⇒ 第一版一直等到超时。
2. **下拉第 4 项采不到**：压缩格式选项来自**服务端** `getFullBackupFormats()`（`available.includes(item)`），
   一次性栈的容器里没装 gzip ⇒ 只有 3 项。第一版硬要求 4 项 ⇒ 把"环境没装 gzip"误判成"没翻"。
   改成：采到的每一项都必须是当前语言的文案 + 至少 3 项 + 🔴 缺 gzip 时**说出来**（skip，不假装验过）。
3. **空态文案采不到**：探针上一步**真的导出过一个备份** ⇒ 表格已有行。改成两种状态都认，
   而且 🔴 **有行时验得更狠**（行内五个动作必须都是当前语言 —— 这比空态文案是更强的证据）。
👉 与 §7.167 C / §7.168 D 同族：**判据要先确认"这个属性在当前状态下到底渲不渲染"**。

#### F. 🔴 既有守卫的 5 处锚点换形状（性质一条没放）
`backupSignatureDownload.test.js`（兜底提示那处 `indexOf` + 「签名」按钮的正则）、
`fullBackup.test.js`（六个列标题 `title: '中文'` → `title: t('<key>', '中文')`）、
`adminRobustness.test.js`（catch 里必须弹「导出失败！」）。
⚠️ 其中列标题那条的正则在**两层转义**里写坏过一次（Python → JS → RegExp 模板串）⇒ 第一次改完仍然红；
👉 又是那条老规矩：**含反斜杠的锚点最容易在多层转义里坏掉**，写完必须**跑一遍**确认它是绿的（不是只看形状对）。

#### G. 基线
- admin `node --test` **772 tests / 169 suites / 0 fail**（+2 = 工具第 6 条 + 接缝那条）；
- 变异对照 **7/7**（按钮退回硬编码 / 🔴 英文接缝空格加回去 / zh-CN 说明改一个字 / 两个模块级注入函数各拿掉一次 t /
  🔴 繁中「還原」改回简体「恢复」/ 语义空操作）；
- 语言包 **1013 key** ×3；`--zh-tw-audit`：1013 key / **736** 个不同汉字 / **0 命中**简体专用字表（例外仍 1 条：`钥`）；
- 棘轮 **78 个文件 / TOTAL 61**（9 条永久例外）；admin 类型门禁 **31/0**；
- 矩阵（5 个阶段全 rc=0）：admin **772/169/0**、守卫 **35 文件 / 3160 条 / 0 失败**、
  jest **288 套件 / 4238 用例（4234 + 4 skip）/ 0 FAIL**、vitest **97 文件 / 1095**、
  server 与 website 的 tsc 各 **0 错**；生产构建 rc=0（`umi.294c5fd9.js` = **1,588,387 B**）；
- 🔴 **真实剩余：53 个文件 / 694 条**；下一块按站长裁定 C 的"大块推进"：`Theme.jsx`(59) →
  `DataManage/**`(134) → `CommentManage/**`(71) → 延期那批(~55) → `About.tsx`(35) → 其余。
- ⚠️ 已知中间态（活体已登记）：系统设置的**页签标签**（`备份恢复` 等 15 个）仍是中文
  ⇒ 属站长裁定 B 解锁的那批，下一轮做。

### 7.170 期 6 第五批（工具批）：`scripts/i18n/wrapTCalls.js` —— 大文件批量接线的**机械部分**交给 AST；以及一次**主动回滚**

**背景**：站长裁定（§7.169 C）改成"大块推进"之后，剩下的都是 50–90 条一个的大文件
（`Backup.jsx` 89 / `Theme.jsx` 59 / `DataManage/Category.jsx` 58 / `BuiltinComments.jsx` 50 / `About.tsx` 35 …）。
手工一条条 `rep(s, old, new)` 有三个代价：① 慢；② 🔴 **锚点撞车**（同一句中文在一个文件里出现多次、或缩进不同
⇒ 本项目"锚点命中 0 次 / 2 次"已踩 **6** 次）；③ 容易漏（漏的那条要到**活体反向判据**才暴露，而那时栈已经建好了）。

#### A. 交付：`scripts/i18n/wrapTCalls.js`（+ 4 条守卫 + 4 条变异对照）
给一张 `{中文原文 → key}` 映射表，按 **AST 位置**把字面量改写成 `t('key', '中文')`：
- **做**三种形状：普通字面量、🔴 **JSX 属性自动补 `{}`**（`label="中文"` → `label={t(...)}`，少一层花括号就是语法错）、
  JSX 文本（保留**同一行内**的前后空白；跨行的按 JSX 折叠规则不留）；可选 `--hook-anchor` 插入 `useIntl` + `t`。
- 🔴 **刻意不做**模板字符串（`` `已转存 ${n} 张` ``）：那需要人决定 ICU 占位符名、以及句子怎么合并
  （本项目已 **8** 次把"模板相加"收成一条整句，每次都是语义判断）⇒ 工具把它们**逐条列出来**（带行号）交给人。
- 🔴 也不碰**注释**与 `console.*` 的实参（与 `bareChinese` 同口径），并且**不重复包**已经是 `t(...)` 的 defaultMessage 位
  （否则逐字对账会红，而且是"改到自己"的经典形状）。
- 🔴 **fail-loud**：`--hook-anchor` 命中不是**恰好 1 次** ⇒ 退出码 3、**不写文件**（不留半成品）；
  写完还会再解析一次，解析不过就打印"请立即 `git checkout --` 回滚"并以退出码 4 失败。
- 输出必须复核：打印"改了几处（literal / jsxAttr / jsxText 各几）/ 模板待手工几处 / **改完还剩几条裸中文**"，
  🔴 "还剩几条"不为 0 就是映射表不全 —— **不要**当成完成。

守卫（`packages/admin/tests/unit/i18nWrapTCalls.test.js`，4 条）+ 变异对照（4/4）：
① 三种形状都改对、且**只改映射表里的**（表外那条中文必须原样留着）；
② 注释 / `console.*` / 已有 `t()` 的 defaultMessage **都不许被改**（M2 把 `!inSkip(nd)` 拿掉 ⇒ 红）；
③ 模板字符串**不动**但必须被列出来；④ 🔴 hook 锚点命中 0 次时**拒绝写文件**（M3 让它照样写 ⇒ 红）。
⚠️ M1 的 `expect` 第一版写的是 `/label=\{t\(/` —— 失败时打印的是**被改坏的源码**（里面是 `label=t(...)`，没有花括号）
⇒ 那个正则永远不中。👉 **变异对照的 expect 要写"失败输出里真的会出现的片段"**（通常就是那条测试的名字）。

#### B. 实测演练（在 `Backup.jsx` 上 dry-run）
3 条映射 ⇒ **改对 5 处**（同一句在文件里出现多次也全中，这正是 AST 相对锚点的优势）、
列出 **8 处模板**待手工（含 `备份-${date}.json`、`${name}（${count} 张表）`、`${n} 上传失败！` 等）、
列出 **86 处未映射**（带行号与种类：StringLiteral / JSXText）。

#### C. 🔴 一次**主动回滚**（记下来是因为它是对的）
本轮确实开始改 `Backup.jsx` 了（手工部分已落地：hook + 8 处模板/片段，bare **89 → 77**），
但剩下的 77 条里有 **37 条是 JSX 文本**，其中十几处是"文字 + 表达式 + 文字"的**片段拼接**：
`将用 <b>X</b> 覆盖：`、`耗时 {n} 秒，备份生成于 {date}`、
`包含 数据库全部集合（…）、 waline 评论库 ，以及 本地静态文件（…）。 拿这一个文件就能…`、
`归档存在服务器的备份目录里（ <code>path</code> 不在静态目录，匿名下载不到 ），下载走后台鉴权接口。`
每一处都要决定 prefix/suffix 怎么切、英文语序怎么排 —— 这是**语义活**，不是机械活。
当时的剩余预算不够把它做完并走完"守卫 + 三语活体 + 变异对照 + 矩阵 + 记账"这一整套
⇒ 🔴 **`git checkout` 回滚 `Backup.jsx` 与那条被我改过的测试锚点**，只留下能独立成立的产出
（工具 + 守卫 + 变异对照 + §7.169 的站长裁定记录）。
👉 依据既有规矩：**永远不要留半成品状态；停在最近的完整状态并写明续做点。**
（若图省事把半迁移的文件提交上去，就会得到一个"半页英文"的备份页 —— 正是本项目一直避免的状态，
而且棘轮要给它挂一条 **77** 的欠条。）

#### D. 续做点（下一轮，勘察已做完）
① 用本工具跑 `Backup.jsx`（映射表 ~77 条，机械部分几分钟）；
② 手工收 **8 处模板** + 十几处 JSX 片段（prefix/suffix 手法：§7.152 B / §7.156 B）；
③ `summarizeTotals` 是**模块级**函数 ⇒ 收尾参 `t = IDENTITY_T`，三个调用点（`data.totals` / `manifest.totals` /
   表格列的 `render`）都要传 t；
④ 🔴 下载文件名 `备份-YYYY-MM-DD.json` 实测**全仓库只有一处**、服务端与恢复流程都**不匹配**它 ⇒ **可以翻译**
   （它不是"服务端产物文件名"那类线路契约，与 §7.162 A 的 `导出说明.md` 不同）；
⑤ 会红的既有锚点已勘定：`backupSignatureDownload.test.js` 的
   `iErr = fn.indexOf("message.error(err?.message || '下载 .sig 失败！')")` ⇒ 改成 `… || t(`（顺序判据本身不用动）；
   `fullBackup.test.js` 的 `/整站备份与恢复/`、`/数据备份（仅数据库记录，JSON）/` 与
   `backupSignatureDownload.test.js` 的 `/不是下载失败/` 都是 **defaultMessage 子串**，改完仍在 ⇒ 不用动。

#### E. 基线
- admin `node --test` **769 tests / 168 suites / 0 fail**（+4 = A 段那组工具守卫）；
- 变异对照 **4/4**；矩阵（5 个阶段全 rc=0）：admin **769/168/0**、守卫 **35 文件 / 3160 条 / 0 失败**、
  jest **288 套件 / 4238 用例（4234 + 4 skip）/ 0 FAIL**、vitest **97 文件 / 1095**、两个 tsc 各 **0 错**、
  类型门禁 **31/0**；
- 语言包 **930 key ×3**（本轮不新增文案，因为 Backup 已回滚）；
- 🔴 **真实剩余：54 个文件 / 783 条**（与上一轮相同 —— 本轮的产出是**工具**与**裁定**，不是覆盖）。

### 7.169 🔴 站长裁定（2026-09-26 21:44）：三件事**解锁**，i18n 转入"大块推进"节奏

站长看了进度汇报（当时实测：**已完成 1,104 / 1,887 条 = 58.5%**，剩 783 条 / 54 文件）之后，明确三条：

| # | 裁定 | 影响 |
|---|---|---|
| **A** | 🔴 `Backup.jsx`(89) 与 `Theme.jsx`(59) **现在就翻，事后站长复核** | 解除"运维类文案必须人工复核后才动"的封印 ⇒ **148 条**解锁（占剩余 19%）。翻完要给出三语活体证据 + 逐条 diff 供复核；**只动文案层，不动备份/恢复逻辑**。 |
| **B** | 🔴 此前按"文档暂不 i18n"延期的一批**现在一起翻**：`utils/walineEmailFields.js`(24) + `components/WalineForm/index.tsx`(13) + 系统配置 11 个外层 / 4 个内层 tab 标签 + `analysisFields.js` 文案（合计约 **55 条**） | 理由（站长认同）：它们是**界面标签/表单提示**，不是文档正文；不翻会让英文下的系统配置页中英混排（此前登记为"已知可见不一致"，现在可以消掉）。⚠️ **文档本身仍然保持中文**（§7.141 A 那条裁定不变），`ANALYSIS_ADMIN_PATH` / `WALINE_ADMIN_PATH` 这类**指向中文文档的 URL 锚点**也仍然是永久例外。 |
| **C** | 🔴 节奏改成**大块推进**：每轮 100–150 条（一整块，如整个 `DataManage/**`），**一轮一次合并三语活体探针**覆盖该轮全部改动面 | 保留的硬规矩一条不减：三语包 key 集合一致、defaultMessage↔zh-CN 逐字对账、棘轮只减不增、**真实浏览器活体证据**、全量矩阵绿、每处改动有变异对照或红转绿。省掉的是**重复的建栈/拆栈**与"逐文件 6–7 条变异对照"（改成按**判据族**抽样，因为那些判据已连续多批被证明承重）。 |

👉 由此，剩余 783 条里**没有"等裁定"的了**：185 条 gated 全部解锁，只有 61 条是**刻意不翻**的永久例外
（服务端产物文件名 / 线路字面量 / 要照着敲的命令 / 插入用户正文的容器模板 / 指向中文文档的 URL 锚点），
其余 ~722 条按 C 的节奏推进（大块顺序见 §7.168 E 的实测排序）。
...
`components/StaticForm/index.tsx`（15 条 → **0**，15 个调用点）；语言包 **462 → 506 key**
（新组 🔴 **`watermark`** 31 条 + 新组 🔴 **`storage`** 13 条）；棘轮清单 **20 → 22 个文件**（都预算 0，**TOTAL 仍 53**）；
`i18nKeyNaming` 下界 → **506**；`localePackParity` 自动发现下界 **17 → 19 个文件 / 470 → 535 个调用点**（实测 19 / 540）；
🔴 **新增一条常驻守卫**：三份包的**数字集合**与**技术标识符**必须一致（见 B）。
🔴 **浏览器活体 25/25（8–9 项判据 × 3 语），problems 0**：11 个 label 全部渲染且逐个等于包里的值、
文本框 placeholder、**4 条长 tooltip 全文逐字相同**（含 52px / 8x8 / 4 个色阶 / 200 字节那些契约数字）、
两个下拉的选项（开启/关闭、WebP（默认）/AVIF）、存储策略 label + 两个选项（本地存储/OSS 图床）+ tooltip；
并且 🔴 **en-US 下这个页签的 label 里一个汉字都没有**（`noCjkLabels` 判据）⇒ **"图床设置"整页不再中英混排**。
🔴 `skipped 18`（**打印出来、不静默**）：6 个 Select 的 placeholder 在活体里观察不到 —— 它们**已有设定值**，
antd 此时渲染的是选中项而不是占位符 ⇒ 这批交给 `localePackParity` 的 defaultMessage↔包对账覆盖。
证据：`vanblog_dev/i18n-browser-evidence/phase5-watermarkform/`。

#### A. 🔴 为什么两个文件一起做：**半个页签的英文比整页中文更糟**
「图床设置」这个页签由 `ImgTab.jsx`（期 3 第一批已翻）+ `WaterMarkForm` + `StaticForm` 三块拼成。
只翻 WaterMarkForm 的话，页面上会**同时**出现「存储策略」（简体）与 "Visible watermark"（英文）——
🔴 那是**活体探针发现的**、不是推演：en-US 的 label 列表末尾明晃晃一个「存储策略」（来自 StaticForm）。
⇒ 同轮把 StaticForm 也做完，并加一条**反向判据**：en-US 下这个页签不许再有汉字 label。
👉 **规矩：一个页面由多个组件拼成时，批次要按"页面"切、不要按"文件"切** ——
按文件切会产出一堆"半英半中"的中间态，而 🔴 那种状态比全中文更难让人相信"这个产品支持英文"。
⚠️ 同类中间态目前还剩三处（都已实测记录，不是猜测）：评论设置页签（7 英 4 中）、定制化页签（4 个内层标签）、
以及本轮新记的一条 —— `watermark.*` 的英文 tooltip 里提到的 `"Images"` / `"Add thumbnails"` /
`"Detect steganographic watermark"` 指向的是 `Static/img` 那个**还没翻**的页面
（父代理裁定：向前一致、写目标语言）⇒ en-US 下会出现"英文说明指向尚未翻译的按钮"，
等 `Static/img`(71 条) 那批落地就自动闭合。

#### B. 🔴 新守卫：**翻译不许改契约**（数字集合 + 技术标识符三份一致）—— 它第一次跑就抓出我自己 6 条
水印那批文案里全是**契约数字**：短边 52px 跳过、缩到 8px 还放不下就跳过、长边小于 320 抬到 320、
每个 8x8 块最多动 4 个色阶、隐写内容最多 200 字节、缩略图默认 300px 约 10KB…
而 `watermarkText.test.js` 那条**跨包钉子**只看**中文源码** ⇒ 🔴 翻译时把 en-US 写成 60px，它**看不见**。
生成语言包的一次性脚本当时比对过数字，但 🔴 **一次性脚本不是守卫**（下一批就没人跑了）⇒ 现在变成常驻断言：
① 每个 key 的**数字集合**（排序后的多重集）三份必须相同；② zh-CN 里出现的**技术标识符**
（全大写词、带扩展名的文件名、`sharp`/`avifenc`/`picgo`/`Waline`/`Caddy` 这类工具名）必须在另两份**原样**出现。
🔴 **它第一次跑就抓出 6 条，逐条读之后分成两类**：
**真缺陷 4 条（已修）** —— `isrManualOk/Fail` 的英文把 **ISR** 这个缩写翻没了；`caddy.redirectLabel` 把 **HTTPS** 写成小写 `https`；
`caddy.confirmOff/On` 把「2 秒」写成 **"Two seconds"**（数字变成单词 ⇒ 任何数字比对都看不见那个 2）；
**尺子太严 1 条（改尺子）** —— `init.restore.err.429`：zh「每 10 分钟 5 次」↔ en "5 per 10 minutes"，
数字没变、只是**语序**变了 ⇒ 判据从"按出现顺序的序列"改成 🔴 **排序后的多重集**（顺序不同是合法的；
少一个数字、或把 52 改成 60 仍然抓得住）。（第 6 条与第 5 条同源，都是 confirmOn/confirmOff。）
👉 **两条规矩**：① 🔴 **"数字/单位/标识符"是文案里的契约，翻译时最容易悄悄漂**，必须有常驻守卫，不能靠一次性脚本；
② 🔴 **新守卫第一次跑出的红要逐条读** —— 里面既有真缺陷也有尺子太严；
一律当成"我的译文有问题"或一律当成"尺子有问题"，两种都会错。
🔴 变异对照两条分别打它：把 en-US 的 `52px` 改成 `60px` ⇒ 红；把 zh-TW 的 `VANBLOG_WATERMARK_POSITION`
改成 `…_POS` ⇒ 红（"缺少必须原样保留的技术标识符"）。

#### C. 🔴 迁移脚本自己的两个坑（都靠"后置断言"才抓住）
1. **删一段代码时连带删掉了写盘语句**：把 SiteInfoForm 版的"结构性前置改造"整段删掉时，
   `fs.writeFileSync` 正好在那一段末尾 ⇒ 脚本打印了"35 个替换点"却**一个字都没改**，
   而后置验证只是 `console.log('bare = 32')` ⇒ 🔴 **脚本 exit 0，看起来像成功**。
   修法：后置验证改成**断言**（bare≠0 / 有调用点没带字面量 defaultMessage / 数量不足 ⇒ `process.exit(1)`）。
   👉 **规矩：迁移脚本的后置验证必须是断言，不能只是打印**（与"探针的跳过必须打印"同族：🔴 **沉默的绿最贵**）。
2. **`{ label: '开启', value: true }` 这种选项被整对跳过**：脚本原本要求 `value` 是**字符串**字面量才收，
   而这里的 value 是 **BooleanLiteral** ⇒ 5 对"开启/关闭"全漏（bare 停在 2）。修法：非字符串就用它的字面值当 slot 名。
   👉 这类漏项**不会报错**，只有"改完立刻用共享模块量 bare"才会显形 ⇒ 那一步不能省。

#### D. 🔴 `label` 是一个 `<a>` 元素时怎么办（StaticForm 的 picgo 配置）
`label={<a href="…docs/features/image-storage.md">picgo 配置</a>}` ⇒ **只翻链接文字**、href 原样保留
（文档按站长裁定仍是中文，与 §7.147 A 那个 URL 锚点同一条纪律）。
⚠️ 通用脚本处理不了这种形状（它只认字符串字面量的 label）⇒ 手工改，并记进"脚本覆盖率不是 100%"的账：
🔴 **机械化迁移之后必须用 `bareChinese` 复核，剩下的那几条就是要手工处理的形状**。

#### E. 术语（已写进 zh-TW 包注释，后续沿用）
**浮水印**（水印）/ **隱寫**（隐写）/ **縮圖**（缩略图）/ **儲存策略**（存储策略）/ **本機儲存**（本地存储）/
**外掛**（插件）/ **自訂**（自定义）/ **對應**（映射）/ **遺失**（丢失）/ **映像**（镜像）/ **工具列**（工具栏）/
**位元組**（字节）/ **載入**（加载）/ **支援**（支持）/ **圖床**（同形）。
⚠️ 英文里 `picgo` / `picgoConfig` / `OSS` / `sharp` / `avifenc` / `libavif-apps` / `WARN` / `GIF` / `JPEG`
是**技术标识符**，三份包都必须原样保留（B 段那条守卫会查）。

#### F. 基线
- admin `node --test` **744 tests / 165 suites / 0 fail**（+1 = B 段那条契约守卫）；
  i18n 守卫组 **100 → 101**（`localePackParity` 44 → **45**）；
- 变异对照 **6/6**（两个文件各一条棘轮、契约守卫两条、key 改名一条、语义空操作一条）；
- 语言包 **506 key** ×3；`--zh-tw-audit`：506 key / **658** 个不同汉字 / **0 命中**简体专用字表（例外仍 1 条：`钥`）；
- 棘轮 **22 个文件 / TOTAL 53**；admin 类型门禁 **31/0**（两个新翻的文件都是 `.tsx`，在门禁范围内，**没加新错**）；
- 矩阵（5 个阶段全 rc=0）：admin **744/165/0**、守卫 **35 文件 / 3160 条 / 0 失败**、
  jest **288 套件 / 4238 用例（4234 + 4 skip）/ 0 FAIL**、vitest **97 文件 / 1095**、
  server 与 website 的 tsc 各 **0 错**；生产构建 rc=0（`umi.12c9ce62.js` = **1,441,461 B**）；
- 🔴 **真实剩余（bareChinese 口径）：108 → 106 个文件 / 1,502 → 1,452 条**（本批 −2 文件 / −50 条）。
- 🔴 **下一批**：① `pages/Static/img/index.tsx`（**71 条**，翻完就能闭合 A 段那个"英文说明指向未翻译按钮"的中间态）；
  ② `pages/Editor/index.jsx`（64 条）；③ `DataManage/tabs/Category.jsx`（58）/ `CommentManage/BuiltinComments.jsx`（50）；
  ④ `SystemConfig` 收尾（`SiteInfo.tsx` 8 / `migrate.tsx` 5；⚠️ SiteInfo 的 3 个内层页签标签属暂缓项）；
  ⑤ 🔴 `Backup.jsx`(89) / `Theme.jsx`(59) 的译文要**交站长人工复核**（备份/恢复/主题属运维高危文案）。
### 7.149 框架：admin 类型门禁**扩到 .jsx**（编译范围 105 → 204 个文件）—— 以及"一条语法错误能让整个门禁假绿"的实测

**交付**：`packages/admin/tsconfig.typecheck.json` 的 `allowJs: false → true` + `include` 加上 `.js`/`.jsx`；
修掉唯一的语法阻塞点（`pages/CommentManage/index.jsx:50` 那 4 个 JSX 文本里的裸 `>`）；
门禁断言 **23 → 31 条**（新增 8 条：语法类必须为 0、`.js/.jsx` 范围下界、`allowJs`/`include`/`checkJs` 三条配置钉、
2 个 `.jsx` 热点文件、`TS2741` 新分类）；基线 **26 → 35**。
🔴 **`checkJs` 刻意不开**：实测开了是 **226 条**（TS2339 76 / TS2322 61 / TS18048 44 / TS2345 13 / TS2304 12 / …），
那是与多语言无关的**独立还债项目** ⇒ 守卫里加了 🔴 **一条断言禁止悄悄打开它**（打开就红，并要求先立项、按 .ts/.tsx 与 .js/.jsx **分两桶**基线）。

#### A. 🔴 实测到一种**能让整个门禁假绿**的形状：一条语法错误 ⇒ tsc 跳过全部语义诊断
把 `.jsx` 纳进编译范围后第一次跑，tsc **只报 4 条**，全是 `CommentManage/index.jsx:50` 的 **TS1382**
（JSX 文本里那 4 个裸 `>`：`站点管理->系统设置->站点配置->高级设置->是否开启评论系统`；**Babel 能忍、tsc 不能**）。
🔴 关键不是"有 4 条语法错"，而是 **tsc 一旦报出语法诊断就不再跑语义诊断** ⇒
那次输出里 🔴 **连既有的 26 条 .ts/.tsx 类型错误、连依赖那 2 条都消失了**。
变异对照把这个形状钉成了常驻证据（T-M1）：把 `{'->'}` 改回裸 `->`，门禁打印的是

```
FAIL: 🔴 出现 4 条 TS1xxx 语法类错误 ⇒ tsc 会跳过整个程序的语义诊断 …
  · admin src 错误 = 4（基线 35）；依赖自带 = 0（基线 2）；合计 4
PASS: 棘轮守住了：admin src 错误 4 < 基线 35（🔴 请把 BASELINE_ADMIN 下调到 4，并在提交信息里写清清了哪几处）
FAIL: 七类之和 == admin src 总数 (got '0', want '4')
```

⇒ 🔴 **棘轮那条是 PASS 的，还建议你"把基线下调到 4"** —— 谁照做，门禁就永久瞎了。
抓住它的只有两条：新增的 **TS1xxx 必须为 0**，和既有的 **"各类之和 == 总数"**（0 ≠ 4）。
👉 **规矩（新）**：类型门禁必须把 🔴 **TS1xxx（语法类）单列成"必须为 0"**，与 TS5xxx/TS6xxx（配置类）同级；
🔴 **"错误数变少"永远不能直接当进步** —— 先确认语义诊断真的跑了。
⚠️ 正则细节：判语法类要用 `error TS1[0-9][0-9][0-9]:`（🔴 **带冒号**），
否则会把 **TS18048**（"可能是 undefined"，实测 44 条）的前 5 位当成 TS1xxx 误报 —— 我第一版就这么量错过一次。

#### B. 🔴 范围变窄会让棘轮"假进步"，所以**范围本身**要钉住
把 `allowJs` 改回 false，错误数会从 35 掉回 26 ⇒ 🔴 **棘轮看起来进步了，实际丢了 99 个文件的覆盖**。
所以除了基线，还加了**四道范围钉**：
① `HOT_FILES` 里放两个 **.jsx**（`CommentManage/index.jsx`、`SystemConfig/tabs/Advance.jsx`）—— 不在编译清单里就红；
② `.js/.jsx` 的**文件数下界 90**（实测 **99**；总 **204** = 105 个 .ts/.tsx + 99 个 .js/.jsx）；
③ 直接读 `tsconfig.typecheck.json` 断言 `allowJs: true`；④ 断言 `include` 里有 `src/**/*.jsx`。
变异对照 T-M2（`allowJs → false`，红 4 条）与 T-M3（include 去掉 `.jsx`，红 4 条）分别打这几条。
👉 **规矩：棘轮类守卫必须同时钉"口径/范围"** —— 否则"把尺子调松"和"把质量提高"在数字上长得一模一样。

#### C. 🔴 扩范围揭露的 9 条真错（已登记为基线，只许减）
`TS2339 7 → **14**`、新增 `TS2741 ×2`（🔴 `UrlFormItem` 的 props 类型要求 `id`，而 `UpdateModal` 两处没传）、
`TS2322 14`（不变）、`TS2345 2`、`TS18048 2`、`TS2538 1`、`TS2769 0`（期 4 清零后保持）⇒ 合计 **35**。
👉 这与"修对配置揭露 6 条被掩盖的真错"同一性质：🔴 **扩大可见范围时基线会先涨一次**，
涨的理由必须逐条写清（哪一类、几条、为什么以前看不见），否则下一个人分不清"新账"和"旧账被照亮"。

#### D. 那句"跨面导航路径词汇"仍未翻（有意）
`CommentManage/index.jsx:50` 的 `站点管理->系统设置->站点配置->高级设置->是否开启评论系统`
属**跨面导航路径词汇**（与 `ANALYSIS_ADMIN_PATH` / `WALINE_ADMIN_PATH` 同族）：
它指向的页签标签本身尚未接 i18n（站长裁定文档暂不做 i18n、页签标签因此暂缓）⇒
🔴 本轮**只修语法、不翻文案**（`{'->'}` 渲染出的字符与原来逐字相同）。
⚠️ 顺带核实过：全仓库只有这一处出现那个字符串（docs 里用的是 `→`）⇒ 没有守卫被这次改写打红。

#### E. 🔴 变异对照**自己的期望**也会写错（第 3 次）
T-M1 第一版的期望抄的是**绿灯**文案（`TS1xxx（语法类）= 0`），而变异体走的是**红灯**文案
（`出现 N 条 TS1xxx 语法类错误`）⇒ harness 判它"没红在该红的地方"（明明红对了）。
这与 §7.143 那两次（期望抄成 TAP 标题、抄成 `244 > 243`）同源 ⇒
🔴 **期望文案必须从"变异体真的跑出来的那份输出"里复制**，不要凭记忆写；
harness 打印"未命中的期望"就是为了让这种错误立刻显形（这次它显形了 ⇒ 机制有效）。

#### F. 🔴 顺带记一条测量陷阱：**"错误总数只有 4"当时看起来像好消息**
第一次带 `.jsx` 跑 tsc 时我看到"总数 4、全是 TS1382"，第一反应是"只有 4 条语法错，太好了" ——
🔴 真相是**语义诊断根本没跑**（既有 26 条一条都没报）。
👉 与手册里那条"TS5xxx/TS6xxx 必须为 0 才能信任何计数"是同一家族，现在补全成三句：
**① 配置类（TS5xxx/6xxx）为 0；② 语法类（TS1xxx）为 0；③ 计数为 0（或远低于基线）时先怀疑测量、再高兴。**

#### G. 🔴 修那 4 个裸 `>` 时又实测到两条**"改法本身"的坑**（都与 JSX 的文本规则有关）
第一版把它改成 `站点管理{'->'}系统设置{'->'}…`（字符逐字保留、看起来等价），实测有两个副作用：
1. 🔴 **一句话被拆成 5 个 JSX 文本节点** ⇒ 裸中文计数从 **1 条变 5 条**，
   `inventory.js` 的"真实剩余"因此从 **1,502 涨到 1,507** —— 🔴 **剩余工作量口径被自己的修法灌了水**。
   这类灌水很危险：它让"进度"看起来倒退，下一个人会去查为什么（我这轮就查了一次）。
2. 🔴 **JSX 会吃掉"文本节点 ↔ 表达式"之间的换行** ⇒ 渲染少一个空格：
   `关闭请前往站点管理…`（原本两行 JSX 文本之间的换行会**折叠成一个空格**；中间一插表达式，那个空格就没了）。
   ⚠️ 这不是理论推演，是改完对比渲染文本发现的，而且 🔴 **没有任何测试会红**（一个空格的差异）。
最终改法：把整句写成**一个字符串表达式** `{'PS: …关闭请前往 站点管理->系统设置->…'}` ⇒
① `>` 在字符串里，TS1382 消失；② 计数回到 **1 条**（真实剩余回到 1,502）；③ 渲染文本与原来逐字相同（空格保留）；
④ 将来翻它时它正好是**一个** key（而不是 5 个碎片）。
👉 **规矩：JSX 文本里要转义标点时，优先把整句收进一个字符串表达式**，不要用 `{'>'}` 把句子切碎 ——
切碎既灌水计数、又可能吃掉空格，而且 🔴 **两者都不会让任何测试变红**。

#### H. 基线
- admin 类型门禁 **31/0**（原 23/0，新增 8 条断言）；🔴 编译范围 **204 个 src 文件**（105 个 .ts/.tsx + **99 个 .js/.jsx**）；
  基线 **admin src 35 / 依赖自带 2**；分类 TS2322 14 / TS2339 14 / TS2769 **0** / TS2345 2 / TS18048 2 / TS2538 1 / TS2741 **2** = 35；
- admin `node --test` **743 tests / 165 suites / 0 fail**；i18n 守卫组仍 **100**（本轮是类型门禁，没动 i18n 断言）；
- 变异对照 **6/6**（T-M1…T-M6）；🔴 其中 T-M1 还额外钉住了"假绿形状"本身
  （断言输出里**必须**出现「棘轮守住了：admin src 错误 4 < 基线 35」与「请把 BASELINE_ADMIN 下调到 4」——
  那就是没有语法类断言时门禁会说的话）；
- 矩阵（**5 个阶段全 rc=0**，且是 I 段修完之后的最终状态）：admin **743/165/0**、
  守卫 **35 文件 / 3160 条 / 0 失败**（+8 = 门禁新增的 8 条断言）、
  jest **288 套件 / 4238 用例（4234 + 4 skip）/ 0 FAIL**、vitest **97 文件 / 1095**、
  server 与 website 的 tsc 各 **0 错**；
- 🔴 **真实剩余（bareChinese 口径）：108 个文件 / 1,502 条**（本轮**没有**翻文案 ⇒ 数字不变；G 段那次 +5 的灌水已修回）。
- 🔴 **下一批**：① 期 5 `components` 大桶（`Static/img` 71、`Editor/index.jsx` 64、`WaterMarkForm` 35…）；
  ② `SystemConfig` 收尾（`SiteInfo.tsx` 8 / `migrate.tsx` 5）—— ⚠️ SiteInfo 的 **3 个内层页签标签**属暂缓项，
  做它会让 TOTAL_BUDGET 从 53 涨到 56（与 Customizing 那 4 条一起等"页签标签 + 文档 i18n"的裁定）；
  ③ 🔴 **`checkJs` 那 226 条要不要立项**（需站长/父代理裁定：它与多语言无关，但不做的话 .jsx **内部**永远没有类型保护）；
  ④ 期 9 批 5：`user.provider` 那 5 处带中文 `label` 参数的模板消息。

#### I. 🔴 顺手修掉 `logRotate.spec.ts` 的**第二类**测量竞态（同一个根因打中了三条用例）
本轮矩阵第二次跑时 jest 又红：`logRotate.spec.ts › 历史文件依次后移，最老的一份被删掉` ——
与上一轮修的那条（`持续写入会反复轮转`）是 🔴 **同一个根因、不同用例**：
`flush()` 只保证"写入缓冲落盘 + 让出一次事件循环"，而 🔴 **轮转是延迟的**
（`maybeRotate()` 要等 `opened === true` 且没有在飞的写入；`rotate()` 里 `createWriteStream` 的 open 又是异步的）
⇒ 负载高时断言会读到"还没转完"的中间状态（`.1` 里是 `bbbb` 而不是 `cccc`）。
🔴 而且**修完那条、单跑这个文件时又红了第三条**（`多字节字符按字节数计`，`rotations` 还是 0）⇒
"逐条打补丁"是**打地鼠**：这个文件里有 **3 条**用例吃同一个竞态。
最终修法：把 11 处 `await s.flush()` 统一换成 `await flushed(s)`（🔴 **等到轮转静止**：连续 5 次≈50ms 次数不变，上限 3s）。
🔴 判据用"静止"而**不是**"等到某个具体次数" —— 期望次数本身就是要断言的东西，用它当等待条件会把断言变成恒真。
修完单跑 **8/8、连续三次绿**。
⚠️ **变异对照在这里不成立（诚实记账）**：把等待去掉后单跑 3/3 都绿 —— 这个竞态**只在全量并行的负载下**出现
⇒ 只能靠"机制读自实现 + 全量跑复现/不复现"来定性，🔴 **不能声称有变异对照**。
🔴 **改这个 helper 时我自己又踩了一个坑**：先插入了 `const flushed = async (s) => { await s.flush(); … }`，
再做全局替换 `await s.flush();` → `await flushed(s);` ⇒ 🔴 **替换命中了 helper 自己那一行** ⇒ 无限递归，
8 条用例全部 `RangeError: Maximum call stack size exceeded`（而且是**确定性**红，一眼看不出是替换造成的）。
👉 **规矩：先做全局替换、再插入 helper**；或者让 helper 里的调用形状与被替换的形状不同。
🔴 这是本仓库"**自己的文本骗过自己的工具**"的第 **5** 次（前四次：注释里的字面量把安全守卫骗绿、
断言匹配到自己的注释、打码标记骗过打码自检、变异对照的期望抄了绿灯文案）。
