...
拼音链接、标题可复制、自动摘要、附件管理、两个 UI 修复、图片管线（缩放/缩略图/隐写水印/列表视图）、整站备份与恢复、单篇文章导出（md + 带图 mdz）。
**别把它们当成脏改动回退掉。**
...
2. 改完代码**必须跑测试**（§2.1），并对照 §7.8 的基线判断是不是自己弄坏的。
