...
/**
 * 前台有 utils/headingText + utils/headingHash，后台没有对应实现，
 * 这里用等价逻辑：标题文本压掉首尾空白与内部换行，再按 URL 片段编码。
 * 注意后台预览里的标题**没有 id**（预览的 Heading 插件只写 data-id），
 * 所以 `[[toc]]` 生成的链接在预览里点不动，发布到前台是正常的。
 */
function normalizeHeadingText(text: string | null | undefined): string {
  return String(text ?? '').replace(/\s+/g, ' ').trim();
}

function headingHashHref(text: string | null | undefined): string {
  return `#${encodeURIComponent(normalizeHeadingText(text))}`;
}

/**
 * 「两边都缺」的 6 种语法一次性补齐（后台编辑器这一份）。
 *
 * ⚠️ 与前台 `packages/website/components/Markdown/extraSyntax.ts` **必须保持一致**
 * （同一套插件、同样顺序、同样的 sanitize 白名单），否则又会出现「预览和发布不一样」。
 * `packages/admin/tests/unit/markdownConsistency.test.js` 会逐项比对两份文件。
 *
