...
export default {
  ...
  // ── 🔴 期 6 第六批（82 keys）── 英文人工写；不用缩写（ICU 把单引号当转义符）。
  //    🔴 计数一律 ICU plural（`{seconds, plural, one {# second} other {# seconds}}` 等 8 处）。
  //    🔴 片段拼接的**接缝空格放在英文值里**（`Includes ` / ` (articles, …), ` / `); downloads go through …`），
  //    中文两份不带 ⇒ 逐字对账仍成立。技术标识符原样保留：zstd -19 --long / xz -9e / gzip -9 / .sig /
  //    .json / JSON / MB / WebP / waline / jwt。
  'backup.compressorAuto': 'Auto (picks the strongest one available on this machine)',
  'backup.compressorZstd': 'zstd -19 --long (smallest and fast, recommended)',
  'backup.compressorXz': 'xz -9e (similar size, several times slower)',
  'backup.compressorGzip': 'gzip -9 (most compatible, slightly larger)',
  'backup.totalsSummary': '{documents, plural, one {# record} other {# records}} / {files, plural, one {# file} other {# files}} / {collections, plural, one {# collection} other {# collections}}',
  'backup.dataFileName': 'backup-{date}.json',
  'backup.exportFailed': 'Export failed!',
  'backup.demoBlockedEdit': 'This cannot be changed on the demo site!',
  'backup.fullBackupDoneTitle': 'Full-site backup created',
  'backup.descFile': 'File',
  'backup.descSize': 'Size',
  'backup.sizePrefix': ' (before compression ',
  'backup.sizeSuffix': ' MB of static files)',
  'backup.descCompressor': 'Compression',
  'backup.descSeconds': 'Duration',
  'backup.secondsValue': '{seconds, plural, one {# second} other {# seconds}}',
  'backup.descTotals': 'Contents',
  'backup.descDatabases': 'Databases',
  'backup.dbEntry': '{name} ({count, plural, one {# collection} other {# collections}})',
  'backup.downloadFailed': 'Download failed!',
  'backup.sigDownloaded': 'Downloaded the .sig: put it and the archive into the **same** off-site copy; the authoritative copy of the public key must be kept **offline** (password manager / printout / another machine)',
  'backup.sigNeverSigned': 'This archive has never been signed (there is no .sig next to it): this is not a download failure. To prove that a copy has not been swapped, generate a key under "Signing key" first, then back up again.',
  'backup.downloadSigFailed': 'Failed to download the .sig!',
  'backup.inspectNoManifest': 'Cannot read the manifest of this backup!',
  'backup.manifestTitle': 'Backup manifest: {name}',
  'backup.descCreatedAt': 'Created at',
  'backup.descCompressorMethod': 'Compression method',
  'backup.descTotal': 'Total',
  'backup.dbNamePrefix': 'Database {name}',
  'backup.staticTitle': 'Static files',
  'backup.staticEntry': '{folder}: {files, plural, one {# file} other {# files}} / {mb} MB',
  'backup.inspectFailed': 'Failed to read the manifest!',
  'backup.restoreConfirmTitle': 'Are you sure you want to overwrite the current site with this backup?',
  'backup.restoreConfirmOk': 'I am sure, restore it',
  'backup.overwritePrefix': 'Will use ',
  'backup.overwriteSuffix': ' to overwrite:',
  'backup.overwriteDatabases': 'All database collections (articles, drafts, categories, tags, image-hosting records, settings, access statistics...)',
  'backup.overwriteWaline': 'The waline comment database',
  'backup.overwriteStatic': 'Local static files (image-hosting images and thumbnails, attachments, custom pages)',
  'backup.overwriteWarning': 'The current data will be replaced and this cannot be undone; it is recommended to export a backup of the current state first.',
  'backup.restoreRelogin': 'After the restore completes you will need to log in again (both the login session and the jwt secret come from the backup).',
  'backup.restoreFailedToast': 'Restore failed!',
  'backup.restoreDoneTitle': 'Restore complete',
  'backup.restoreDoneSummary': 'Took {seconds, plural, one {# second} other {# seconds}}; the backup was created at {createdAt}',
  'backup.restoreDbLine': '{name}: {collections, plural, one {# collection} other {# collections}} / {documents, plural, one {# record} other {# records}}',
  'backup.restoreStaticLine': 'Static files {folder}: {files, plural, one {# file} other {# files}}',
  'backup.deletedToast': 'Deleted',
  'backup.deleteFailed': 'Failed to delete!',
  'backup.colName': 'Backup file',
  'backup.colFormat': 'Format',
  'backup.actDownload': 'Download',
  'backup.downloadSigBtn': 'Download .sig (offline signing)',
  'backup.actSignature': 'Sign',
  'backup.actManifest': 'Manifest',
  'backup.actRestore': 'Restore',
  'backup.deleteConfirmTitle': 'Delete this backup file?',
  'backup.fullSectionTitle': 'Full-site backup and restore',
  'backup.fullSectionHint': 'One archive = the entire blog',
  'backup.includesPrefix': 'Includes',
  'backup.includesDatabases': 'all database collections',
  'backup.includesDatabasesDetail': ' (articles, drafts, categories, tags, image-hosting records, settings, access statistics), ',
  'backup.includesAnd': ', and',
  'backup.includesStatic': 'local static files',
  'backup.includesStaticDetail': ' (image-hosting images and thumbnails, attachments, custom pages). With just this one file you can restore the entire blog on a new machine.',
  'backup.compressorNote': 'The compression format defaults to the strongest one available on this machine (usually zstd -19); the images themselves are already WebP, so the overall compression ratio mostly depends on the database part. The archive is stored in the backup directory on the server (',
  'backup.archiveNoteBold': 'not in the static directory, so it cannot be downloaded anonymously',
  'backup.archiveNoteSuffix': '); downloads go through the authenticated admin endpoint.',
  'backup.formatLabel': 'Compression format:',
  'backup.exportFullBtn': 'Export full-site backup',
  'backup.restoreSecondsOnly': 'Took {seconds, plural, one {# second} other {# seconds}}.',
  'backup.restoreFailedTitle': 'Restore failed',
  'backup.restoreIncomplete': 'The upload succeeded but the restore did not complete; check the server logs.',
  'backup.uploadFailedFull': 'Failed to upload {name}!',
  'backup.uploadRestoreBtn': 'Upload backup and restore',
  'backup.refreshBtn': 'Refresh list',
  'backup.fullEmpty': 'No full-site backup yet; click "Export full-site backup" above to create one',
  'backup.dataSectionTitle': 'Data backup (database records only, JSON)',
  'backup.dataSectionNote': 'Note: importing does not overwrite the current admin login account. This JSON import/export "does not include images, attachments, or the comments themselves"; it only includes image records for retrieval. To back up the files as well, use the "Full-site backup" above.',
  'backup.demoBlockedReason': 'Someone put pornographic content on the demo site homepage, so this permission has been disabled.',
  'backup.uploadOkRefresh': '{name} uploaded successfully! Refresh in a moment and it will take effect!',
  'backup.importAllBtn': 'Import all data',
  'backup.exportAllBtn': 'Export all data',
  'backup.includesWaline': 'the waline comment database',
