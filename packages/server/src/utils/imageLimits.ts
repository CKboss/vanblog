 ...
 * 而 `POST /api/admin/img/upload` 在 `types/access/access.ts` 的 `publicRoutes`（②免权限档）里
 * ⇒ **勾了至少一项权限的**协作者也能触发（⚠️ 2026-09-22 更正：原文写"最低权限的协作者"，
 * 而 `AccessGuard` 拆两层之后，**零权限**协作者已经打不到这条；能打的是"有任意一项权限"的协作者）。
 * 这是纵深防御：不是当前可利用的洞，
