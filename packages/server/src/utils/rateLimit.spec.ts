...
// 🔴 2026-09-21 修：原来这里是 `Math.floor(Math.random() * 200) + 20`，即**从 200 个地址里随机取**。
//    本文件有 14 处调用、每处要打几十发请求，而限流桶是**按 IP** 计的 ⇒ 一旦两次调用撞上同一个 IP，
//    两条用例就会**共享同一个桶**，于是"某条用例的额度被另一条吃掉了" ⇒ 偶发红。
//    实测复现过：全量并行跑时 `GET /api/public/health 不占 init 桶的额度` 那条红，
//    单独连跑 2 次都是 23/23 全绿 —— 这正是"红自己消失"的形状，但根因**不是负载，是取值会撞**。
//    ⚠️ 这类红很容易被误记成负载假红而永久豁免（本仓库已栽过两次）；改成递增序号后**进程内绝不重复**。
let ipSeq = 0;
const uniqueIp = () => {
  ipSeq += 1;
  if (ipSeq > 199) throw new Error(`uniqueIp 超出 203.0.113.20-219 的容量（${ipSeq}），请加网段而不是改回随机`);
  return `203.0.113.${ipSeq + 19}`;
};
