...
#                          事实恰好相反：18080 在本机上已经被另一套长期运行的栈占着**（端口清单见
#                          AGENTS.local.md），而本脚本 :241 会 `-p "${SMOKE_HTTP_PORT}:80"` 真的去 bind 它。
#                          也就是说**跑一次带冒烟的构建就会去抢那个端口**：被占时 podman 会失败（构建白跑一趟），
#                          而那套栈一旦在重启窗口里，就可能被这个临时容器占住。
#                          ⚠️ 站长裁定（2026-09-21）：那套栈是**演练栈**、不是生产站，所以
#                          `scripts/benchmark/measure.sh` 把压测**打向**它是**有意的**、不用改。
