...
doctor() {
  ...
  # 🔴 **用之前必须先截断**（2026-09-22 修）：这个文件名是**固定的**（只按 `$$` 区分），
  #    而它**从不清理**、且 🔴 **PID 会被复用** ⇒ 某次运行可能撞上一个**上一次留下的同名文件**。
  #    如果这一次 curl 又**写不出**响应体（只读文件系统、TMPDIR 不可写 —— 正是下面 `body_ok`
  #    那一支存在的理由），`[[ -s "${body}" ]]` 就会因为那个**陈旧文件**为真 ⇒ `body_ok=1` ⇒
  #    doctor 会**把上一次的健康状态当成当前结论**自信地报出来，而"诊断不了"那一支被完全绕过。
  #    截断之后两个方向都对：写得出来 ⇒ 内容就是本次的；写不出来 ⇒ `: >` 也失败、`-s` 仍为假 ⇒
  #    一定落到 `body_ok=0`。⚠️ `|| true` 是必要的：临时目录不可写时这一句本身就会失败，
  #    而那恰恰是我们要如实报告的情形，不能让它中断 doctor。
